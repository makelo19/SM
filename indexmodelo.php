...
$sysSettings = $config->getSystemSettings();
...
if (isset($_POST['acao']) && strpos($_POST['acao'], 'ajax_') === 0) {
    ...
    if ($act == 'ajax_get_process_filter_options') {
        ...
        foreach ($fields as $f) {
            if (!empty($f['is_filter'])) {
                $key = $f['key'];
                $vals = [];
                
                // If options defined in schema, use them
                if (($f['type'] == 'select' || $f['type'] == 'multiselect') && !empty($f['options'])) {
                    $vals = array_map('trim', explode(',', $f['options']));
                } 
                
                // If empty or explicitly needed from data (e.g. Atendente is text but we want options)
                // For performance, we only scan data if it's NOT a fixed option list OR if options are empty.
                // But for "text" fields that are filters, we might not want a dropdown unless it's Atendente.
                // Let's scan data if type is select/multiselect and options empty, OR if it is "Nome_atendente".
                
                if (empty($vals) && ($f['type'] == 'select' || $f['type'] == 'multiselect' || $key == 'Nome_atendente')) {
                     foreach ($files as $file) {
                         $v = $db->getUniqueValues($file, $key);
    ...
    if ($act == 'ajax_list_anexos') {
        $port = $_POST['port'] ?? '';
        if (!$port) { echo json_encode(['status'=>'error']); exit; }
        
        // Sanitize
        $port = preg_replace('/[^a-zA-Z0-9_-]/', '_', $port);
        $dir = $uploadDir . $port . '/';
    ...
    if ($act == 'ajax_upload_anexo') {
        ...
        if (!$port || empty($_FILES['file'])) { 
            echo json_encode(['status'=>'error', 'message'=>'Arquivo inválido.']); exit; 
        }
    ...
    if ($act == 'ajax_render_dashboard_table') {
        ...
        foreach ($_POST as $k => $v) {
            if (strpos($k, 'filter_') === 0 && $v !== '') {
                $fieldKey = substr($k, 7);
                // Handle special chars or encoding if needed, but keys are usually safe
                $filters[$fieldKey] = $v;
            }
        }
        ...
        if ($fBusca) {
             ...
             $filters['callback'] = function($row) use ($fBusca, $foundPorts, $checkDate) {
        ...
        if ($sortCol === 'Nome' || $sortCol === 'Nome_atendente') {
            ...
            usort($processos, function($a, $b) use ($sortCol, $desc) {
                if ($sortCol === 'Nome') {
                    $nameA = get_value_ci($a, 'Nome') ?: '';
                    $nameB = get_value_ci($b, 'Nome') ?: '';
        ...
        foreach($processos as $proc) {
            ...
            $nome = get_value_ci($proc, 'Nome') ?: 'N/A';
            ...
            if($cred) echo ' <i class="' . htmlspecialchars($GLOBALS['sysSettings']['currency_icon'] ?? 'fas fa-coins') . ' money-bag ms-2" title="Identificação!"></i>';
            echo '</td>';
    ...
    if ($act == 'ajax_salvar_campo') {
        ...
        $isFilter = isset($_POST['is_filter']) ? true : false;
    ...
    if ($act == 'ajax_save_settings') {
        $settings = [
            'system_name' => $_POST['system_name'],
            'currency_icon' => $_POST['currency_icon']
        ];
        $current = $config->getSystemSettings();
        $new = array_merge($current, $settings);
        $config->saveSystemSettings($new);
        ob_clean(); header('Content-Type: application/json');
    ...
    if ($act == 'ajax_render_config') {
        global $sysSettings; // Ensure access
        ob_start();
        ?>
        <div class="card card-custom p-4 mb-5">
            <h5 class="text-navy mb-3">Configurações Gerais</h5>
            <form onsubmit="event.preventDefault(); saveSystemSettings(this);">
                <div class="row g-3">
                    <div class="col-md-6">
                        <label>Nome do Sistema</label>
                        <input type="text" name="system_name" class="form-control" value="<?= htmlspecialchars($sysSettings['system_name'] ?? '') ?>">
                    </div>
                    <div class="col-md-6">
                        <label>Ícone de Identificação (FontAwesome)</label>
                        <input type="text" name="currency_icon" class="form-control" value="<?= htmlspecialchars($sysSettings['currency_icon'] ?? 'fas fa-coins') ?>">
                    </div>
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center <?= $liClass ?>" data-key="<?= htmlspecialchars($f['key']) ?>">
                            <div>
                                ...
                                <?php if(!$isTitle): ?>
                                    ...
                                    <?php if($f['is_filter'] ?? false): ?><i class="fas fa-filter text-success ms-1" title="Filtro Ativo"></i><?php endif; ?>
                                <?php else: ?>
...
if (!isset($_SESSION['logado'])) {
    ...
    <head>
        ...
        <title>Login - <?= htmlspecialchars($sysSettings['system_name'] ?? 'SM') ?></title>
    ...
    <body>
        <div class="login-card">
            <h3 class="mb-4" style="color: #003366;"><?= htmlspecialchars($sysSettings['system_name'] ?? 'SM') ?></h3>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?= htmlspecialchars($sysSettings['system_name'] ?? 'SM') ?></title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
...
<nav class="navbar navbar-expand-lg navbar-custom mb-4 sticky-top">
    <div class="container-fluid">
        <a class="navbar-brand" href="?p=dashboard" onclick="showPage('dashboard'); return false;"><i class="fas fa-chart-pie me-2"></i><?= htmlspecialchars($sysSettings['system_name'] ?? 'SM') ?></a>
            ...
            <ul class="navbar-nav me-auto">
                ...
                <li class="nav-item d-flex align-items-center ms-2">
                    ...
                    <div class="tab-pane fade" id="tab-anexos">
                        <div class="card card-custom p-4 mb-4">
                            <h5 class="text-navy mb-3"><i class="fas fa-paperclip me-2"></i>Anexos</h5>
                            <div class="mb-3">
                                <label class="form-label">Adicionar Anexo</label>
                                <div class="input-group">
                                    <input type="file" class="form-control" id="upload_anexo_file">
                                    <button class="btn btn-navy" type="button" onclick="uploadAnexo()">Upload</button>
                                </div>
                            </div>
                            <div class="table-responsive">
                                <table class="table table-sm table-hover">
                                    <thead><tr><th>Nome</th><th>Tamanho</th><th>Data</th><th>Ações</th></tr></thead>
                                    <tbody id="anexos_table_body"></tbody>
                                </table>
                            </div>
                        </div>
                    </div>
            ...
            <form onsubmit="filterDashboard(event)" class="row g-3" id="form_dashboard_filter">
                ...
                foreach($procFields as $f) {
                    if (!empty($f['is_filter'])) {
                        ...
                        if (($f['type'] == 'select' || $f['type'] == 'multiselect') || $f['key'] == 'Nome_atendente') {
                            echo '<select name="filter_' . htmlspecialchars($f['key']) . '" id="filter_' . htmlspecialchars($f['key']) . '" class="form-select form-select-sm dynamic-filter-select">';
                            echo '<option value="">Todos</option>';
                            echo '</select>';
                        } else {
                            echo '<input type="text" name="filter_' . htmlspecialchars($f['key']) . '" class="form-control form-control-sm">';
                        }
                        echo '</div>';
                    }
                }
                ...
                <div class="col-md-2"><label>Busca Global</label><input type="text" name="fBusca" class="form-control form-control-sm" placeholder="Busca Global..." value="<?= htmlspecialchars($fBusca) ?>"></div>
                <div class="col-12 text-end">
        ...
        <div class="row">
            ...
            <div class="col-md-12">
                <ul class="nav nav-tabs mb-3 justify-content-center">
                    ...
                    <li class="nav-item"><button type="button" class="nav-link" data-bs-toggle="tab" data-bs-target="#tab-anexos" onclick="loadAnexos()">Anexos</button></li>
                </ul>
                                ...
                                $val = $getVal($processo, $f['key']);
                                    ...
                                    <div class="col-md-4 <?= $hideClass ?>" data-field-key="<?= $f['key'] ?>">
                                        ...
                                        <?php if($f['key'] == 'Numero_Portabilidade'): ?>
                                            <div class="input-group">
                                                <input type="text" name="Numero_Portabilidade" id="proc_port" class="form-control form-control-custom fw-bold" value="<?= htmlspecialchars($val ?: ($id ?? '')) ?>" required placeholder="Buscar/Criar por ID...">
    ...
    <div class="modal-dialog">
        <form class="modal-content" method="POST" onsubmit="event.preventDefault(); submitConfigForm(this, 'ajax_salvar_campo')">
            ...
            <div class="modal-body">
                ...
                <div class="form-check mb-3" id="div_field_is_filter">
                    <input class="form-check-input" type="checkbox" name="is_filter" id="field_is_filter">
                    <label class="form-check-label" for="field_is_filter">Usar como Filtro</label>
                </div>
            </div>
            <div class="modal-footer"><button class="btn btn-navy">Salvar</button></div>
        </form>
...
<script>
    ...
    function editField(f, file) { 
        ...
        document.getElementById('field_is_filter').checked = (f.is_filter === true || f.is_filter === "true");
        document.getElementById('field_is_filter').checked = false;
        new bootstrap.Modal(document.getElementById('modalField')).show(); 
    }
    ...
    function saveSystemSettings(form) {
        ...
        setButtonLoading(btn, true);
        var fd = new FormData(form);
        fd.set('acao', 'ajax_save_settings');
    ...
    function loadFilterOptions() {
        var selects = document.querySelectorAll('.dynamic-filter-select');
        ...
        .then(res => {
            if(res.status == 'ok') {
                selects.forEach(sel => {
                    var key = sel.id.replace('filter_', '');
                    var opts = res.options[key];
                    if (opts) {
                        // Clear except first
    ...
    function loadAnexos() {
        var port = document.getElementById('proc_port').value;
        if (!port) {
            document.getElementById('anexos_table_body').innerHTML = '<tr><td colspan="4" class="text-center text-muted">Salve o processo primeiro.</td></tr>';
            return;
        }
        ...
        .then(res => {
            if(res.status == 'ok') {
                var html = '';
                if(res.files.length === 0) html = '<tr><td colspan="4" class="text-center text-muted">Nenhum anexo.</td></tr>';
                res.files.forEach(f => {
                    var size = (f.size / 1024).toFixed(2) + ' KB';
                    html += '<tr><td>' + f.name + '</td><td>' + size + '</td><td>' + f.time + '</td><td>';
                    html += '<button class="btn btn-sm btn-link text-primary" onclick="renameAnexo(\'' + f.name + '\')"><i class="fas fa-pen"></i></button>';
                    html += '<button class="btn btn-sm btn-link text-danger" onclick="deleteAnexo(\'' + f.name + '\')"><i class="fas fa-trash"></i></button>';
                    html += '</td></tr>';
                });
                document.getElementById('anexos_table_body').innerHTML = html;
            }
        });
    }
    ...
    function uploadAnexo() {
        var port = document.getElementById('proc_port').value;
        var fileInput = document.getElementById('upload_anexo_file');
        
        if (!port) { Swal.fire('Erro', 'Salve o processo primeiro.', 'error'); return; }
        if (fileInput.files.length === 0) { Swal.fire('Erro', 'Selecione um arquivo.', 'error'); return; }
        ...
        .catch(() => { hideLoading(); Swal.fire('Erro', 'Falha no envio.', 'error'); });
    }

    function deleteAnexo(name) {
        var port = document.getElementById('proc_port').value;
        Swal.fire({ title: 'Excluir Anexo?', showCancelButton: true, confirmButtonText: 'Sim' }).then(r => {
            if(r.isConfirmed) {
                var fd = new FormData();
    ...
    function renameAnexo(oldName) {
        var port = document.getElementById('proc_port').value;
        Swal.fire({ title: 'Novo Nome', input: 'text', inputValue: oldName, showCancelButton: true }).then(r => {
            if(r.isConfirmed && r.value) {
                var fd = new FormData();
                fd.append('acao', 'ajax_rename_anexo');
                fd.append('port', port);
                fd.append('old_name', oldName);
                fd.append('new_name', r.value);
                fetch('', { method: 'POST', body: fd }).then(r=>r.json()).then(res => {
                    if(res.status=='ok') loadAnexos();
                    else Swal.fire('Erro', res.message, 'error');
                });
