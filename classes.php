...
class Templates {
    ...
    public function __construct($identIdField = 'PORTABILIDADE') {
        $this->identIdField = $identIdField;
        if (!file_exists($this->file)) {
            $defaults = [
    ...
    public function getHistory($port) {
        ...
        foreach ($data as $idx => $row) {
            if (isset($row[$this->identIdField]) && $row[$this->identIdField] == $port) {
    ...
    public function deleteHistoryItems($ids) {
        if (empty($ids)) return false;
        if (!file_exists($this->historyFile)) return false;
        ...
        if ($modified) {
             file_put_contents($this->historyFile, json_encode($newData, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
             return true;
        }
        return false;
    }
