...
class SystemConfig {
    private $file = 'dados/system_config.json';
    private $config = [];

    public function __construct() {
        if (!file_exists($this->file)) {
            $this->config = [
                'app_name' => 'SM - Sistema Modular',
                'pk_field' => 'Numero_Portabilidade', 
                'currency_symbol' => 'R$'
            ];
            // Ensure dir exists
    ...
    public function getAppName() { return $this->config['app_name'] ?? 'SM - Sistema Modular'; }
    public function setAppName($v) { $this->config['app_name'] = $v; $this->save(); }

    public function getPKField() { return $this->config['pk_field'] ?? 'Numero_Portabilidade'; }
    public function setPKField($v) { $this->config['pk_field'] = $v; $this->save(); }

    public function getCurrencySymbol() { return $this->config['currency_symbol'] ?? 'R$'; }
    public function setCurrencySymbol($v) { $this->config['currency_symbol'] = $v; $this->save(); }
    ...
    public function importProcessData($newData) {
        ...
        foreach ($groups as $key => $rows) {
             ...
             foreach ($existing as $r) {
                 $rPk = get_value_ci($r, $this->pkField);
                 if ($rPk) {
                     $dataMap[$rPk] = $r;
                 }
             }
             ...
             foreach ($rows as $row) {
                 $pk = get_value_ci($row, $this->pkField);
                 if ($pk) {
                     if (isset($dataMap[$pk])) {
...
class Config {
    ...
    public function getDashboardFields($file) {
        $fields = $this->getFields($file);
        $res = [];
        foreach ($fields as $f) {
            // Check for explicit true or string "true" (from POST)
            $v = $f['visible_in_dashboard'] ?? false;
            if ($v === true || $v === 'true' || $v === 'on' || $v === 1) {
                $res[] = $f;
            }
        }
        // If empty, return defaults? Or handle in UI? 
        // Better to return defaults if nothing is configured to avoid empty table.
        // Default defaults: DATA, Nome, CPF, Numero_Portabilidade, STATUS, VALOR DA PORTABILIDADE, Nome_atendente
        if (empty($res)) {
             // Return defaults only if this is Processos
             if (strpos($file, 'Processos') !== false || strpos($file, 'Base_processos') !== false) {
                 $sys = new SystemConfig();
                 $pk = $sys->getPKField();
                 $defaults = ['DATA', 'Nome', 'CPF', $pk, 'VALOR DA PORTABILIDADE', 'STATUS', 'Nome_atendente'];
                 foreach ($fields as $f) {
                     // Check if key is in defaults
                     if (in_array($f['key'], $defaults)) { // Should be CI check
                         $res[] = $f;
                     } else {
                         // CI check
                         foreach ($defaults as $d) {
                             if (strcasecmp($d, $f['key']) === 0) {
                                 $res[] = $f;
                                 break;
                             }
                         }
    ...
    public function getFilterFields($file) {
        $fields = $this->getFields($file);
        $res = [];
        foreach ($fields as $f) {
            $v = $f['visible_in_filter'] ?? false;
            if ($v === true || $v === 'true' || $v === 'on' || $v === 1) {
                $res[] = $f;
            }
        }
        if (empty($res)) {
             if (strpos($file, 'Processos') !== false || strpos($file, 'Base_processos') !== false) {
                 // Defaults
                 $defaults = ['Nome_atendente', 'STATUS'];
                 foreach ($fields as $f) {
                     if (in_array($f['key'], $defaults)) $res[] = $f;
                 }
             }
        }
        return $res;
...
class AttachmentManager {
    ...
    public function __construct($baseDir = 'dados/anexos/') {
    ...
    private function getSafePath($port) {
        $safePort = preg_replace('/[^a-zA-Z0-9_-]/', '_', $port);
        return $this->baseDir . $safePort;
    }

    public function listFiles($port) {
        $dir = $this->getSafePath($port);
        $files = [];
        if (is_dir($dir)) {
    ...
    public function upload($port, $file) {
        $dir = $this->getSafePath($port);
    ...
    public function delete($port, $name) {
        $dir = $this->getSafePath($port);
    ...
    public function rename($port, $oldName, $newName) {
        $dir = $this->getSafePath($port);
    ...
    public function getFilePath($port, $name) {
        $dir = $this->getSafePath($port);
        $path = $dir . '/' . basename($name);
        if (file_exists($path) && realpath($path) && strpos(realpath($path), realpath($this->baseDir)) === 0) {
            return $path;
        }
        return null;
    }
    ...
    public function rebuild($db) {
        ...
        foreach ($files as $f) {
            ...
            if (file_exists($path)) {
                ...
                foreach ($data as $row) {
                    $port = get_value_ci($row, $this->pkField);
                    if ($port) {
                        $this->index[$port] = $f;
                    }
                }
