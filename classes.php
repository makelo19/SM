...
class Settings {
    ...
    private function getDefaults() {
        return [
            ...
            'id_field_key' => 'Numero_Portabilidade',
            'id_label' => 'Identificação',
            'id_search_label' => 'Buscar por Identificação',
            'currency_symbol' => 'R$',
            'currency_icon' => 'fas fa-coins',
            'identification_label' => 'Identificação',
            'identification_icon' => 'fas fa-fingerprint',
            'identification_id_field' => 'PORTABILIDADE',
            'value_field_key' => 'VALOR DA PORTABILIDADE',
            
            // Process Lists (Dashboard)
            'dashboard_columns' => [
                ['key' => 'Numero_Portabilidade', 'label' => 'Identificação', 'sortable' => true],
                ['key' => 'Nome', 'label' => 'Nome', 'sortable' => true],
                ['key' => 'CPF', 'label' => 'CPF', 'sortable' => true],
                ['key' => 'STATUS', 'label' => 'Status', 'sortable' => true]
            ],
            'visible_process_filters' => ['STATUS'],
            
            // Base Lists (Registros)
            'base_columns' => [
                 ['key' => 'STATUS', 'label' => 'Status', 'sortable' => false],
                 ['key' => 'NUMERO_DEPOSITO', 'label' => 'Nº Depósito', 'sortable' => false]
            ],
            'base_filters' => ['STATUS'],
    ...
    public function getIdFieldKey() {
        return $this->get('id_field_key', 'Numero_Portabilidade');
    }
    ...
    public function getIdentificationIdField() {
        return $this->get('identification_id_field', 'PORTABILIDADE');
    }
...
class Database {
    private $dataDir;
    public $processIdField = 'Numero_Portabilidade';
    ...
    public function importExcelData($file, $newData) {
        ...
        if (empty($headers)) {
             $headers = ['STATUS', 'NUMERO_DEPOSITO', 'DATA_DEPOSITO', 'VALOR_DEPOSITO_PRINCIPAL', 'TEXTO_PAGAMENTO', $this->identIdField, 'CERTIFICADO', 'STATUS_2', 'CPF', 'AG'];
        }
    ...
    private function init() {
        if (!file_exists($this->configFile)) {
            ...
            $files = [
                ...
                'Identificacao.json' => [
                     ...
                     $this->db->identIdField => 'text', 'CERTIFICADO' => 'text', 'STATUS_2' => 'text', 
                     'CPF' => 'text', 'AG' => 'text'
                ],
                'Base_processos_schema' => [
                    $this->db->processIdField=>'text', 'Certificado'=>'text', 
            ...
            foreach ($files as $file => $defaultTypes) {
                ...
                if ($file == 'Base_processos_schema') {
                     ...
                     $headers = ['DATA', 'Ocorrencia', 'Status_ocorrencia', 'Nome_atendente', $this->db->processIdField, 'CPF', 'AG', 'Certificado', 'PROPOSTA', 'AUT_PROPOSTA', 'STATUS', 'OBSERVAÇÃO'];
                } else {
        ...
        if (isset($data['Identificacao.json']) && empty($data['Identificacao.json'])) {
            $data['Identificacao.json'] = [
                 ...
                 ['key'=>'NUMERO_DEPOSITO', 'label'=>'Número Identificação', 'type'=>'text'],
                 ['key'=>'DATA_DEPOSITO', 'label'=>'Data', 'type'=>'date'],
                 ['key'=>'VALOR_DEPOSITO_PRINCIPAL', 'label'=>'Valor Principal', 'type'=>'money'],
                 ['key'=>'TEXTO_PAGAMENTO', 'label'=>'Observação', 'type'=>'textarea'],
