...
.navbar-superior-fixa .titulo-nav { color: #fff; font-size: 18px; font-weight: bold; margin-right: 15px; white-space: nowrap; }
.navbar-superior-fixa .usuario-nav { color: #fff; font-size: 14px; font-weight: 600; white-space: nowrap; }

.metric-selector-nav { display: flex; gap: 5px; }
.m-btn-nav { 
    background: rgba(255,255,255,0.1); color: #ccc; border: 1px solid rgba(255,255,255,0.2); 
    padding: 6px 10px; border-radius: 6px; cursor: pointer; font-weight: 600; font-size: 11px;
    transition: all 0.2s; white-space: nowrap;
}
.m-btn-nav:hover { background: rgba(255,255,255,0.2); color: white; }
.m-btn-nav.active { background: #f7a049; color: #fff; border-color: #f7a049; }

.action-buttons-nav { display: flex; gap: 5px; }
.btn-nav {
    border: none; padding: 6px 10px; border-radius: 6px; cursor: pointer; font-weight: bold; font-size: 11px;
    transition: opacity 0.2s; white-space: nowrap; color: white; display: flex; align-items: center; gap: 5px;
}
.btn-nav:hover { opacity: 0.9; }
.btn-excel-nav { background: #1d7044; }
.btn-refresh-nav { background: var(--success); }
.btn-clear-nav { background: var(--danger); }
...
th { background: var(--primary); padding: 15px; text-align: left; font-size: 11px; color: #fff; text-transform: uppercase; cursor: pointer; } 
...
/* --- NOVOS ESTILOS PARA BANDEJAS EXPANSÍVEIS --- */
.tray-container { background: #fff; border-radius: 12px; border: 1px solid #e5e7eb; margin-top: 25px; overflow: hidden; }
.tray-header { display: flex; justify-content: space-between; align-items: center; padding: 15px 20px; cursor: pointer; background: #fff; border-bottom: 1px solid #f1f5f9; transition: background 0.2s; }
.tray-header:hover { background: #f8fafc; }
.tray-title { font-weight: 800; font-size: 16px; margin: 0; color: #1e293b; }
.tray-icon { transition: transform 0.3s; color: #64748b; }
.tray-content { display: none; padding: 0; }
.tray-container.expanded .tray-content { display: block; }
.tray-container.expanded .tray-icon { transform: rotate(180deg); }
...
<div class="navbar-superior-fixa">
    <div class="nav-left">
        ...
        <div class="metric-selector-nav"> 
            <button class="m-btn-nav active" id="m_qtd" onclick="setMetric('qtd')">Volumetria (Qtd)</button> 
            <button class="m-btn-nav" id="m_val" onclick="setMetric('val')">Valores (R$)</button> 
            <button class="m-btn-nav" id="m_tma" onclick="setMetric('tma')">TMA Intervalo (Min)</button> 
        </div>
    </div>
    <div class="nav-right">
        <div class="action-buttons-nav">
            <button class="btn-nav btn-excel-nav" onclick="openExcelModal()"><i class="fas fa-file-excel"></i> Excel</button> 
            <button class="btn-nav btn-refresh-nav" onclick="init()"><i class="fas fa-sync-alt"></i> Atualizar</button> 
            <button class="btn-nav btn-clear-nav" onclick="resetAll()"><i class="fas fa-eraser"></i> Limpar</button> 
        </div>
...
<div class="tray-container expanded">
    <div class="tray-header" onclick="toggleTray(this)">
        <h3 class="tray-title">Estrutura Hierárquica</h3>
        <i class="fas fa-chevron-down tray-icon"></i>
    </div>
...
<div class="tray-container expanded">
    <div class="tray-header" onclick="toggleTray(this)">
        <h3 class="tray-title">Histórico de Registros</h3>
        <i class="fas fa-chevron-down tray-icon"></i>
    </div>
    <div class="tray-content">
        <table id="history-table"> 
            <thead>
                <tr>
                    <th onclick="sortHistory('_ts')">Data e Hora <i class="fas fa-sort"></i></th>
                    <th onclick="sortHistory('Tipo/Fechamento')">Tipo/Fechamento <i class="fas fa-sort"></i></th>
                    <th onclick="sortHistory('Categoria')">Categoria <i class="fas fa-sort"></i></th>
                    <th onclick="sortHistory('Número da Ocorrência')">Número da Ocorrência <i class="fas fa-sort"></i></th>
                    <th onclick="sortHistory('Título(s)')">Título(s) <i class="fas fa-sort"></i></th>
                    <th onclick="sortHistory('Tema(s)')">Tema(s) <i class="fas fa-sort"></i></th>
                    <th onclick="sortHistory('usuarioLogado')">Usuário Logado <i class="fas fa-sort"></i></th>
                </tr>
            </thead> 
            <tbody></tbody> 
        </table> 
...
let historySort = { col: '_ts', dir: 'desc' };
...
function formatValue(v) { 
    ...
    if(activeMetric === 'tma') {
        ...
        return isNaN(val) ? "0 min" : val + " min";
    }
    return Math.round(v); 
} 
...
function updateKPIs() { 
    const getU = (data) => { 
        ...
        if (uniqueArr.length > 1) {
             const first = uniqueArr[0]._ts;
             const last = uniqueArr[uniqueArr.length - 1]._ts;
             const diff = last - first; // diferença em segundos
             const avgSeconds = diff / (uniqueArr.length - 1);
             totalMinutos = avgSeconds / 60;
        } else {
             totalMinutos = 0;
        }

        // Garante que não retorne NaN se totalMinutos não for um número válido
        if (isNaN(totalMinutos)) totalMinutos = 0;

        return { q: uniqueArr.length, v: totalVal, t: Math.round(totalMinutos) }; 
    }; 
    ...
    const setK = (id, obj) => { 
        const tVal = isNaN(obj.t) ? 0 : obj.t;
        document.getElementById(`k_${id}_main`).innerText = activeMetric === 'val' ? money(obj.v) : (activeMetric === 'tma' ? tVal+" min" : obj.q); 
        document.getElementById(`k_${id}_sub`).innerText = `R$: ${money(obj.v)} | TMA: ${tVal} min`; 
    }; 
...
function setMetric(m) { activeMetric = m; document.querySelectorAll('.m-btn-nav').forEach(b => b.classList.remove('active')); document.getElementById('m_'+m).classList.add('active'); renderAll(); } 
...
function renderAll() { updateKPIs(); renderUserChart(); renderThemeChart(); renderMatrix(); renderHistoryTable(); } 

function toggleTray(header) {
...
function renderHistoryTable() {
    ...
    dataToSort.sort((a, b) => {
        ...
        // Check if numeric (and not just numeric string unless intended)
        // For 'Número da Ocorrência', try parsing as number
        if (historySort.col === 'Número da Ocorrência' || historySort.col === '_ts') {
             valA = Number(valA);
             valB = Number(valB);
        }

        if (valA < valB) return historySort.dir === 'asc' ? -1 : 1;
        if (valA > valB) return historySort.dir === 'asc' ? 1 : -1;
        return 0;
    });
    ...
    tbody.innerHTML = dataToSort.map(item => `
        <tr>
            <td>${formatarDataBR(item['Data'])}</td>
            <td>${item['Tipo/Fechamento'] || ''}</td>
            <td>${item['Categoria'] || ''}</td>
            <td>${item['Número da Ocorrência'] || ''}</td>
            <td>${item['Título(s)'] || ''}</td>
            <td>${item['Tema(s)'] || ''}</td>
            <td>${item['usuarioLogado'] || ''}</td>
        </tr>
    `).join('');
    
    // Update header icons
    document.querySelectorAll('#history-table th i').forEach(icon => icon.className = 'fas fa-sort');
    const activeHeader = Array.from(document.querySelectorAll('#history-table th')).find(th => th.getAttribute('onclick').includes(historySort.col));
    if (activeHeader) {
        const icon = activeHeader.querySelector('i');
        if (icon) icon.className = historySort.dir === 'asc' ? 'fas fa-sort-up' : 'fas fa-sort-down';
    }
}
