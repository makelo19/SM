        ...
        if ($originalPort && $originalPort !== $port) {
             ...
             if ($oldFile) {
                 ...
                 if ($oldData) {
                     ...
                     // 3. Rename Attachments (Migration to Immutable ID)
                     $existingRef = isset($oldData['_anexos_ref']) ? $oldData['_anexos_ref'] : null;
                     
                     if (!$existingRef) {
                         $safeOld = preg_replace('/[^a-zA-Z0-9_\-\.]/', '', $originalPort);
                         $oldDir = 'upload/' . $safeOld;
                         
                         if ($safeOld && is_dir($oldDir)) {
                             // Migrate legacy folder to new unique structure
                             $dateStr = date('dmY');
                             $uniquePart = uniqid() . mt_rand(1000,9999);
                             $newDirName = $dateStr . $uniquePart;
                             $newDir = 'upload/' . $newDirName;
                             
                             if (rename($oldDir, $newDir)) {
                                 $fullData['_anexos_ref'] = $newDir;
                             } else {
                                 // Fallback: try renaming to new ID if migration fails (legacy preservation)
                                 $safeNew = preg_replace('/[^a-zA-Z0-9_\-\.]/', '', $port);
                                 if ($safeNew) {
                                     rename($oldDir, 'upload/' . $safeNew);
                                 }
                             }
                         }
                     }
...
if (isset($_GET['acao']) && $_GET['acao'] == 'exportar_excel') {
    ...
    $allRegistros = [];
    foreach ($processos as $proc) {
        $pPort = get_value_ci($proc, $ID_FIELD_KEY);
        if (!in_array($pPort, $ports)) continue;
        
        // Get _historico_envios from the process file
        $pFile = $indexer->get($pPort);
        if (!$pFile) {
            $pFile = $db->findFileForRecord($db->getAllProcessFiles(), $ID_FIELD_KEY, $pPort);
        }
        if ($pFile) {
            $hist = $db->getProcessSubData($pFile, $ID_FIELD_KEY, $pPort, '_historico_envios');
            if (is_array($hist)) {
                foreach ($hist as $h) {
                    $h['_port'] = $pPort;
                    $allHistorico[] = $h;
            ...
            $regs = $db->getProcessSubData($pFile, $ID_FIELD_KEY, $pPort, '_registros_processo');
            if (is_array($regs)) {
                foreach ($regs as $r) {
                    $r['_port'] = $pPort;
                    $allRegistros[] = $r;
