...
if (isset($_POST['acao']) && strpos($_POST['acao'], 'ajax_') === 0) {
    ...
    if ($act == 'ajax_upload_file') {
        ...
        if (!$targetDir) {
             echo json_encode(['status'=>'error', 'message'=>'Falha ao criar diretório de anexos.']); 
             exit;
        }
...
if (!isset($_SESSION['logado'])) {
    ...
    <script>document.addEventListener("DOMContentLoaded", function() { var inputs = ["field_label", "field_key"]; inputs.forEach(function(id) { var el = document.getElementById(id); if (el) { el.addEventListener("input", function(e) { if(id === "field_label") { var typeEl = document.getElementById("field_type"); if(typeEl && typeEl.value === "title") return; } this.value = this.value.replace(/\s/g, ""); }); } }); });</script></body>
    </html>
    <?php
    exit;
}
...
if (isset($_GET['acao']) && $_GET['acao'] == 'exportar_excel') {
    ...
    $fComIdentificador = $_GET['fComIdentificador'] ?? '';
    if ($fComIdentificador) {
        $originalCallback = $filters['callback'] ?? null;
        $allIdentificacoes = $db->select('Identificacao.json', [], 1, 999999);
        $identIdsMap = [];
        if (!empty($allIdentificacoes['data'])) {
            foreach ($allIdentificacoes['data'] as $i) {
                $idVal = get_value_ci($i, $IDENT_ID_FIELD);
                if ($idVal !== null && $idVal !== '') {
                    $identIdsMap[(string)$idVal] = true;
                    $identIdsMap[mb_strtoupper(trim((string)$idVal), 'UTF-8')] = true;
                }
            }
        }
        $filters['callback'] = function($row) use ($originalCallback, $identIdsMap, $ID_FIELD_KEY) {
            if ($originalCallback && !$originalCallback($row)) return false;
            $pId = get_value_ci($row, $ID_FIELD_KEY);
            if ($pId === null || $pId === '') return false;
            
            $pIdStr = (string)$pId;
            if (isset($identIdsMap[$pIdStr])) return true;
            
            $pIdNorm = mb_strtoupper(trim($pIdStr), 'UTF-8');
            return isset($identIdsMap[$pIdNorm]);
        };
    }
    ...
    // Read consolidated history data from process records for export
    // OPTIMIZED: Read each process file only ONCE and extract both sub-keys simultaneously
    $allHistorico = [];
    $allRegistros = [];
    $portsFlip = array_flip($ports); // O(1) lookups instead of in_array O(N)
    foreach ($targetFile as $tf) {
        $tfPath = $db->getPath($tf);
        if (!file_exists($tfPath)) continue;
        $tfRows = $db->readJSON($tfPath);
        foreach ($tfRows as $row) {
            $pPort = isset($row[$ID_FIELD_KEY]) ? $row[$ID_FIELD_KEY] : '';
            if ($pPort === '') {
                foreach ($row as $rk => $rv) {
                    if (strcasecmp($rk, $ID_FIELD_KEY) === 0) { $pPort = $rv; break; }
                }
            }
            if ($pPort === '' || !isset($portsFlip[$pPort])) continue;
            if (isset($row['_historico_envios']) && is_array($row['_historico_envios'])) {
                foreach ($row['_historico_envios'] as $h) {
                    $h['_port'] = $pPort;
                    $allHistorico[] = $h;
                }
            }
            if (isset($row['_registros_processo']) && is_array($row['_registros_processo'])) {
                foreach ($row['_registros_processo'] as $r) {
                    $r['_port'] = $pPort;
                    $allRegistros[] = $r;
                }
            }
    ...
    foreach ($allHistorico as $row) {
        if (isset($row['TYPE']) && $row['TYPE'] === 'EMAIL') {
            $valData = $clean($row['DATA'] ?? '');
            $valPort = $clean($row['_port'] ?? '');
            $valUser = $clean($row['USUARIO'] ?? '');
            $valAssunto = $clean($row['ASSUNTO'] ?? '');
            $valPara = $clean($row['PARA'] ?? '');
            $valCc = $clean($row['CC'] ?? '');
    ...
    // Read from consolidated _historico_envios
    foreach ($allHistorico as $row) {
        if (isset($row['TYPE']) && $row['TYPE'] === 'EMAIL') continue;
        $valData = $clean($row['DATA'] ?? '');
        $valPort = $clean($row['_port'] ?? '');
        $valUser = $clean($row['USUARIO'] ?? '');
    ...
    // Read from consolidated _registros_processo
    foreach ($allRegistros as $row) {
...
<head>
    ...
    <style>
        ...
        .table-custom th, .table-custom td { font-size: 11px !important; }
        .sortable-header { font-size: 11px !important; }
        .status-badge { padding: 5px 10px; border-radius: 12px; font-size: 0.75em; font-weight: bold; }
        .dashboard-text { font-size: 11px !important; }
...
<div class="container-fluid px-4">
    ...
    <div id="page-dashboard" class="page-section" style="<?= $page=='dashboard'?'':'display:none' ?>">
        ...
        // Extra fetch se precisar de identificar se tem Identificador
        if ($fComIdentificador) {
            $originalCallback = $filters['callback'] ?? null;
            $allIdentificacoes = $db->select('Identificacao.json', [], 1, 999999);
            $identIdsMap = [];
        ...
        $cpfs = [];
        $ports = [];
        foreach($dashProcessos as $row) {
            $c = get_value_ci($row, 'CPF'); if ($c !== null && $c !== '') $cpfs[] = $c;
            $p = get_value_ci($row, $ID_FIELD_KEY); 
            if ($p !== null && $p !== '') {
                $pStr = (string)$p;
                $ports[] = $pStr;
                $ports[] = mb_strtoupper(trim($pStr), 'UTF-8');
                $ports[] = mb_strtolower(trim($pStr), 'UTF-8');
            }
        }
        $clientes = $db->findMany('Base_clientes.json', 'CPF', array_unique($cpfs));
        $clientMap = []; foreach ($clientes as $c) $clientMap[$c['CPF']] = $c['Nome'];
        $dashCreditos = $db->findMany('Identificacao.json', $IDENT_ID_FIELD, array_unique($ports));
        $dashCreditoMap = []; 
        foreach ($dashCreditos as $c) {
            $idVal = get_value_ci($c, $IDENT_ID_FIELD);
            if ($idVal !== null && $idVal !== '') {
                $dashCreditoMap[(string)$idVal] = $c;
                $dashCreditoMap[mb_strtoupper(trim((string)$idVal), 'UTF-8')] = $c;
            }
        }
        ?>
        <div class="card card-custom p-4 mb-4">
            ...
            <form onsubmit="filterDashboard(event)" class="row g-3" id="form_dashboard_filter">
                ...
                <div class="col-md-2 d-flex align-items-end">
                    <div class="form-check mb-1">
                        <input class="form-check-input" type="checkbox" value="1" id="fComIdentificador" name="fComIdentificador" <?= !empty($fComIdentificador) ? 'checked' : '' ?> onchange="filterDashboard(event)">
                        <label class="form-check-label small" for="fComIdentificador">
                            Somente c/ Identificador
                        </label>
                    </div>
                </div>
                ...
                <table class="table table-hover table-custom align-middle" id="dash_table_head">
                    ...
                    <tbody id="dash_table_body">
                        <?php foreach($dashProcessos as $proc): 
                            ...
                            $procIdNorm = $procIdVal !== null ? mb_strtoupper(trim((string)$procIdVal), 'UTF-8') : '';
                            $cred = isset($dashCreditoMap[(string)$procIdVal]) || ($procIdNorm !== '' && isset($dashCreditoMap[$procIdNorm])); 
                    ...
                    <div class="tab-pane fade show active" id="tab-dados">
                        ...
                        <form method="POST" id="form_processo">
                            ...
                            foreach($procFields as $f): 
                                ...
                                if (strcasecmp(trim($f['key']), 'Data_Lembrete') === 0) {
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                ...
                                <div class="d-flex align-items-center" style="width: 250px;">
                                    <div class="input-group input-group-sm">
                                        ...
                                        <input type="text" id="tpl_search" class="form-control border-start-0 ps-0" placeholder="Buscar palavra-chave..." oninput="filterTemplatesSearch(this.value)">
                                    </div>
                                </div>
                                <div class="d-flex align-items-center gap-2">
    ...
    function clearDashboardFilters() {
        ...
        if(form) {
            form.querySelectorAll('input, select').forEach(el => {
                if(el.type != 'hidden' && el.type != 'button' && el.type != 'submit' && el.type != 'checkbox') {
                    el.value = '';
                }
            });
...
function highlightText(text, keyword) {
    ...
    // Remove tags HTML antes para não quebrar (básico)
    var escapedText = String(text).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    var regex = new RegExp("(" + keyword.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + ")", "gi");
    return escapedText.replace(regex, "<mark style='background-color: yellow; color: black; padding: 0;'>$1</mark>");
}

var currentSearchKeyword = '';

function filterTemplatesSearch(keyword) {
    currentSearchKeyword = keyword.trim().toLowerCase();
    renderTemplatesUI();
    
    // Auto-expand/collapse based on search
    if (currentSearchKeyword) {
        // Expand all lists that have visible items
        document.querySelectorAll('#templates_container .accordion-collapse').forEach(function(collapseEl) {
            var hasVisibleItems = collapseEl.querySelectorAll('li[style=""]').length > 0 || collapseEl.querySelectorAll('li:not([style*="display: none"])').length > 0;
            if (hasVisibleItems) {
                collapseEl.classList.add('show');
                var headerBtn = collapseEl.previousElementSibling.querySelector('.accordion-button');
                if (headerBtn) headerBtn.classList.remove('collapsed');
            }
        });
        // Auto-show all previews that are visible and contain matches
        document.querySelectorAll('#templates_container .tpl-preview-area').forEach(function(previewDiv) {
            if (previewDiv.parentElement.style.display !== 'none') {
        ...
        document.querySelectorAll('#templates_container .accordion-collapse').forEach(function(collapseEl) {
            ...
            var headerBtn = collapseEl.previousElementSibling.querySelector('.accordion-button');
            if (headerBtn) headerBtn.classList.add('collapsed');
        });
        // Hide all previews
        document.querySelectorAll('#templates_container .tpl-preview-area').forEach(function(previewDiv) {
...
function createTemplateItem(t, currentChecked) {
    ...
    let rawCorpo = t.corpo || '';
    let safeCorpo = rawCorpo.replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/'/g,'&#39;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    
    // Logic for filtering and highlighting
    let displayStyle = '';
    let displayTitle = t.titulo;
    let displayCorpo = safeCorpo;
    
    // Add current keyword state access via global currentSearchKeyword
    if (typeof currentSearchKeyword !== 'undefined' && currentSearchKeyword !== '') {
        let matchTitle = t.titulo.toLowerCase().includes(currentSearchKeyword);
        let matchTema = (t.tema && t.tema.toLowerCase().includes(currentSearchKeyword));
        let matchCorpo = rawCorpo.toLowerCase().includes(currentSearchKeyword);
        
        if (!matchTitle && !matchTema && !matchCorpo) {
            displayStyle = 'display: none;';
        } else {
            if (matchTitle) displayTitle = highlightText(t.titulo, currentSearchKeyword);
            if (matchCorpo) displayCorpo = highlightText(rawCorpo, currentSearchKeyword);
        }
    }
    
    return `
    <li class="list-group-item bg-transparent border-bottom p-0" style="${displayStyle}">
        ...
        <div class="tpl-preview-area" id="tpl_preview_${t.id}" style="display:none; padding: 8px 16px 12px 40px;">
            <div style="background: #f0f4f8; border-left: 3px solid #001f3f; border-radius: 4px; padding: 10px 12px; font-size: 0.85rem; white-space: pre-wrap; word-wrap: break-word; color: #333; max-height: 200px; overflow-y: auto;" data-corpo="${safeCorpo}">${displayCorpo}</div>
        </div>
    </li>
    `;
}
