...
$config->ensureField('Base_processos_schema', ['key'=>'STATUS', 'label'=>'STATUS', 'type'=>'select', 'options'=>'EM ANDAMENTO, CONCLUÍDO, CANCELADO, ASSINADO', 'required'=>true]);
...
if (isset($_POST['acao']) && strpos($_POST['acao'], 'ajax_') === 0) {
    ...
    if ($act == 'ajax_delete_greeting') {
        ...
        ob_clean(); header('Content-Type: application/json');
        $greetings->delete($id);
        echo json_encode(['status'=>'ok', 'message'=>'Excluído com sucesso.']);
        exit;
    }
    ...
    if ($act == 'ajax_set_default_greeting') {
        ...
        echo json_encode(['status'=>'ok', 'message'=>'Padrão definido.']);
        exit;
    }
    ...
    // 4. Salvar Histórico (Email/Envios)
    if ($act == 'ajax_save_history') {
        ...
        $newRecord = [
            ...
            $IDENT_ID_FIELD => $port, // Keeps compatibility
            'MODELO' => $_POST['modelo'] ?? '',
            'TEXTO' => $_POST['texto'] ?? '',
        ...
        if (!$file) {
             ob_clean(); header('Content-Type: application/json');
             echo json_encode(['status' => 'error', 'message' => 'Processo não encontrado para salvar histórico.']);
             exit;
        }

        $success = $db->modifyJSON($db->getPath($file), function($rows) use ($port, $ID_FIELD_KEY, $newRecord) {
            $updated = false;
            foreach ($rows as &$proc) {
                $procPort = get_value_ci($proc, $ID_FIELD_KEY);
                if ($procPort == $port) {
                    if (!isset($proc['historico_envios']) || !is_array($proc['historico_envios'])) {
                        $proc['historico_envios'] = [];
                    }
                    $proc['historico_envios'][] = $newRecord;
                    
                    // Update main process timestamp
                    $proc['Data'] = date('d/m/Y H:i'); 
                    $proc['Nome_atendente'] = $_SESSION['nome_completo'];
                    $updated = true;
                    break;
                }
            }
            return $updated ? $rows : false;
        });

        if ($success) {
            ob_clean(); header('Content-Type: application/json');
            echo json_encode(['status' => 'ok', 'data' => $newRecord['DATA'], 'usuario' => $newRecord['USUARIO'], 'id' => $newRecord['UID']]);
        } else {
            ob_clean(); header('Content-Type: application/json');
            echo json_encode(['status' => 'error', 'message' => 'Erro ao salvar histórico.']);
        }
        exit;
    }
    ...
    if ($act == 'ajax_upload_file') {
        ...
        for ($i = 0; $i < $fileCount; $i++) {
            ...
            // Adiciona a ID (port) ao nome do anexo se ainda não estiver presente
            if (strpos($fileName, $port . '_') !== 0 && strpos($fileName, $port . '-') !== 0) {
                $fileName = $port . '_' . $fileName;
            }
        ...
        if ($successCount > 0) {
            // Update Process with Attachment References
            $file = $indexer->get($port);
            if (!$file) {
                 $years = $_SESSION['selected_years'] ?? [date('Y')];
                 $months = $_SESSION['selected_months'] ?? [(int)date('n')];
            ...
            if ($file) {
                $db->modifyJSON($db->getPath($file), function($rows) use ($port, $ID_FIELD_KEY, $uploadedFiles) {
                    foreach ($rows as &$proc) {
                        if (get_value_ci($proc, $ID_FIELD_KEY) == $port) {
                            if (!isset($proc['anexos']) || !is_array($proc['anexos'])) {
                                $proc['anexos'] = [];
                            }
                            // Append new files
                            ...
                            // Normalize annexes (dedup by name, keep latest)
                            $unique = [];
                            foreach ($proc['anexos'] as $idx => $a) {
                                $unique[$a['name']] = $a;
                            }
                            $proc['anexos'] = array_values($unique);
                            return $rows;
                        }
                    }
                    return false;
    ...
    if ($act == 'ajax_list_attachments') {
        ...
        // Try reading from Process Object first (for Metadata)
        $processAnexos = null;
        $file = $indexer->get($port);
        if (!$file) {
             $years = $_SESSION['selected_years'] ?? [date('Y')];
             $months = $_SESSION['selected_months'] ?? [(int)date('n')];
             $filesSearch = $db->getProcessFiles($years, $months);
             $file = $db->findFileForRecord($filesSearch, $ID_FIELD_KEY, $port);
        }
        
        if ($file) {
            $proc = $db->find($file, $ID_FIELD_KEY, $port);
            if ($proc && isset($proc['anexos']) && is_array($proc['anexos'])) {
                $processAnexos = $proc['anexos'];
            }
        }

        if ($processAnexos !== null) {
            // Verify physical existence
            foreach ($processAnexos as $a) {
                $path = $targetDir . $a['name'];
                if (file_exists($path)) {
                    $size = filesize($path);
            ...
            // Legacy / Fallback: Scan Directory
            if (is_dir($targetDir)) {
    ...
    if ($act == 'ajax_download_zip') {
        ...
        if (!$port) {
            ob_clean(); header('Content-Type: application/json');
            echo json_encode(['status'=>'error', 'message'=>'ID do processo inválido.']);
            exit;
        }
    ...
    if ($act == 'ajax_delete_attachment') {
        ...
        if (file_exists($targetFile)) {
            if (unlink($targetFile)) {
                ...
                $procFile = $indexer->get($port);
                if (!$procFile) {
                     $years = $_SESSION['selected_years'] ?? [date('Y')];
                     $months = $_SESSION['selected_months'] ?? [(int)date('n')];
                     $files = $db->getProcessFiles($years, $months);
                     $procFile = $db->findFileForRecord($files, $ID_FIELD_KEY, $port);
                }
                
                if ($procFile) {
                    $db->modifyJSON($db->getPath($procFile), function($rows) use ($port, $ID_FIELD_KEY, $file) {
                        foreach ($rows as &$proc) {
                            if (get_value_ci($proc, $ID_FIELD_KEY) == $port) {
                                if (isset($proc['anexos']) && is_array($proc['anexos'])) {
                                    $proc['anexos'] = array_filter($proc['anexos'], function($a) use ($file) {
                                        return $a['name'] !== $file;
                                    });
                                    $proc['anexos'] = array_values($proc['anexos']); // Reindex
                                }
                                return $rows;
                            }
                        }
    ...
    if ($act == 'ajax_rename_attachment') {
        ...
        if (file_exists($dir . $oldName)) {
            if (rename($dir . $oldName, $dir . $newName)) {
                ...
                if ($procFile) {
                    $db->modifyJSON($db->getPath($procFile), function($rows) use ($port, $ID_FIELD_KEY, $oldName, $newName) {
                        foreach ($rows as &$proc) {
                            if (get_value_ci($proc, $ID_FIELD_KEY) == $port) {
                                if (isset($proc['anexos']) && is_array($proc['anexos'])) {
                                    foreach($proc['anexos'] as &$a) {
                                        if ($a['name'] === $oldName) {
                                            $a['name'] = $newName;
                                        }
                                    }
    ...
    if ($act == 'ajax_save_process_data_record') {
        ...
        if (!$file) {
             ob_clean(); header('Content-Type: application/json');
             echo json_encode(['status' => 'error', 'message' => 'Processo não encontrado para salvar registro.']);
             exit;
        }

        $success = $db->modifyJSON($db->getPath($file), function($rows) use ($port, $ID_FIELD_KEY, $record, $isUpdate, $uid) {
            $updated = false;
            foreach ($rows as &$proc) {
                $procPort = get_value_ci($proc, $ID_FIELD_KEY);
                if ($procPort == $port) {
                    if (!isset($proc['registros_processo']) || !is_array($proc['registros_processo'])) {
                        $proc['registros_processo'] = [];
                    }
                    
                    if ($isUpdate) {
                        foreach ($proc['registros_processo'] as &$r) {
                            $rId = isset($r['UID']) ? $r['UID'] : md5(json_encode($r));
                            if ($rId === $uid) {
                        ...
                        $proc['registros_processo'][] = $record;
                    }

                    // Update timestamp on process itself
                    $proc['Ultima_Alteracao'] = date('d/m/Y H:i:s');
                    $proc['Nome_atendente'] = $_SESSION['nome_completo'];
                    $updated = true;
                    break;
                }
        ...
        if ($success) {
            ...
            $processData = $db->find($file, $ID_FIELD_KEY, $port);
            $registros = isset($processData['registros_processo']) ? $processData['registros_processo'] : [];
            
            // Legacy Merge (Optional, for view consistency if needed, but saving is now exclusive)
            // If user wants to see old records immediately after save, we might need to fetch them too.
            // However, the prompt implies moving storage. Assuming existing data will be merged on READ.
            // For now, return what we just saved + embedded history.
            
            // Also fetch legacy records to display them combined
            $legacyRegistros = [];
            if (file_exists($db->getPath('Base_registros_dados.json'))) {
                 $legacyAll = $db->readJSON($db->getPath('Base_registros_dados.json'));
                 foreach($legacyAll as $lRow) {
                     if (($lRow[$IDENT_ID_FIELD]??'') == $port) {
                         $legacyRegistros[] = $lRow;
                     }
                 }
            }
            
            $allRegistros = array_merge($legacyRegistros, $registros);
            
            // Sort by Date Descending
            usort($allRegistros, function($a, $b) {
                 $da = get_value_ci($a, 'DATA');
                 $db = get_value_ci($b, 'DATA');
                 
                 $tA = 0; $tB = 0;
                 $dtA = DateTime::createFromFormat('d/m/Y H:i', $da);
                 if ($dtA) $tA = $dtA->getTimestamp();
                 $dtB = DateTime::createFromFormat('d/m/Y H:i', $db);
                 if ($dtB) $tB = $dtB->getTimestamp();
            ...
            foreach($allRegistros as &$reg) {
                $reg['_id'] = isset($reg['UID']) ? $reg['UID'] : md5(json_encode($reg));
            }

            ob_clean(); header('Content-Type: application/json');
            echo json_encode(['status' => 'ok', 'history' => $allRegistros]);
        } else {
            ob_clean(); header('Content-Type: application/json');
            echo json_encode(['status' => 'error', 'message' => 'Erro ao salvar registro no processo.']);
        }
        exit;
    }
        ...
        if ($originalPort && $originalPort !== $port) {
             ...
             if ($oldFile) {
                 ...
                 if ($oldData) {
                     ...
                     // 2. Delete Old
                     $db->delete($oldFile, $ID_FIELD_KEY, $originalPort);
                     $indexer->delete($originalPort);
                     
                     // 3. Rename Attachments
                     $safeOld = preg_replace('/[^a-zA-Z0-9_\-\.]/', '', $originalPort);
                     $safeNew = preg_replace('/[^a-zA-Z0-9_\-\.]/', '', $port);
                     if ($safeOld && $safeNew && is_dir('upload/' . $safeOld)) {
                         rename('upload/' . $safeOld, 'upload/' . $safeNew);
                     }
                     ...
                     $msg = "Processo renomeado e salvo com sucesso!";
                     $renamed = true;
                 }
             }
        }
 ...
 usort($processos, function($a, $b) use ($sortCol, $desc, $clientMap) {
    ...
    if ($act == 'ajax_render_lembretes_table') {
        ...
        if ($needsRecords && !empty($rows)) {
            // 1. Check Embedded Records
            foreach ($rows as $row) {
                $p = get_value_ci($row, $ID_FIELD_KEY);
                if (isset($row['registros_processo']) && is_array($row['registros_processo'])) {
                    foreach ($row['registros_processo'] as $rec) {
            ...
            // 2. Check Legacy Records
            $ports = array_column($rows, $ID_FIELD_KEY);
            if(!empty($ports) && file_exists($db->getPath('Base_registros_dados.json'))) {
    ...
    if ($act == 'ajax_get_process_full') {
        ...
        // 1. Merge Registros (Embedded + Legacy)
        $registrosHistory = isset($process['registros_processo']) ? $process['registros_processo'] : [];
        ...
        // Sort DESC
        usort($registrosHistory, function($a, $b) {
             $da = get_value_ci($a, 'DATA');
             $db = get_value_ci($b, 'DATA');
        ...
        foreach($registrosHistory as &$row) {
            $row['_id'] = isset($row['UID']) ? $row['UID'] : md5(json_encode($row));
        }

        // 2. Merge Email History (Embedded + Legacy)
        $emailHistory = isset($process['historico_envios']) ? $process['historico_envios'] : [];
        $legacyEmails = $templates->getHistory($port);
        $emailHistory = array_merge($legacyEmails, $emailHistory); // Legacy first usually? Or sort.
        
        // Sort Emails DESC
        usort($emailHistory, function($a, $b) {
             $da = get_value_ci($a, 'DATA');
             $db = get_value_ci($b, 'DATA');
...
if (!isset($_SESSION['logado'])) {
    ...
    <script>document.addEventListener("DOMContentLoaded", function() { var inputs = ["field_label", "field_key"]; inputs.forEach(function(id) { var el = document.getElementById(id); if (el) { el.addEventListener("input", function(e) { this.value = this.value.replace(/\s/g, ""); }); } }); });</script></body>
    </html>
    <?php
    exit;
}
...
if (isset($_GET['acao']) && ($_GET['acao'] == 'download_base' || $_GET['acao'] == 'download_identificacao_full')) {
    ...
    // Prioritize Config to ensure Dynamic Fields
    $headers = [];
    $confFields = $config->getFields($base);
    foreach($confFields as $f) {
        if (($f['type'] ?? '') !== 'title' && !($f['deleted'] ?? false)) {
            $headers[] = $f['key'];
        }
    }
    ...
    $schemaKey = $base;
    if ($base === 'Processos') $schemaKey = 'Base_processos_schema';
    $baseSchema = $config->getFields($schemaKey);
    ...
    $statusKey = 'STATUS';
    $atendenteKey = 'Nome_atendente';
    foreach ($baseSchema as $f) {
        if (mb_strtoupper($f['key']) === 'STATUS') $statusKey = $f['key'];
        if (in_array(mb_strtoupper($f['key']), ['ATENDENTE', 'NOME_ATENDENTE'])) $atendenteKey = $f['key'];
    }
    ...
    if (!empty($_GET['fAtendente'])) $dynamicRules[$atendenteKey] = $_GET['fAtendente'];
    if (!empty($_GET['fStatus'])) $dynamicRules[$statusKey] = $_GET['fStatus'];
    ...
    $checkDynamicBase = function($row) use ($dynamicRules, $fieldMap, $fieldConfigMap) {
         foreach($dynamicRules as $k => $v) {
             ...
             if ($fType === 'date' || $fType === 'datetime') {
                 if (!$rowVal) return false;
                 $dRow = DateTime::createFromFormat('d/m/Y', $rowVal);
                 if (!$dRow) $dRow = DateTime::createFromFormat('Y-m-d', $rowVal);
                 if (!$dRow) $dRow = DateTime::createFromFormat('d/m/Y H:i:s', $rowVal);
                 if (!$dRow) $dRow = DateTime::createFromFormat('Y-m-d H:i:s', $rowVal);
    ...
    $fDataIni = $_GET['cfDataIni'] ?? ($_GET['fDataIni'] ?? '');
    $fDataFim = $_GET['cfDataFim'] ?? ($_GET['fDataFim'] ?? '');
    ...
    if ($fDataIni || $fDataFim) {
          ...
          if ($fDataIni) {
              $di = DateTime::createFromFormat('Y-m-d\TH:i', $fDataIni);
              if (!$di) $di = DateTime::createFromFormat('!Y-m-d', $fDataIni);
              if (!$di) $di = DateTime::createFromFormat('!d/m/Y', $fDataIni);
          }
          if ($fDataFim) {
              $df = DateTime::createFromFormat('Y-m-d\TH:i', $fDataFim);
              if (!$df) {
          ...
          $filters['callback'] = function($row) use ($di, $df, $dateCol, $checkDynamicBase) {
             ...
             if (!$d) return false;
              ...
              if (!$dt) $dt = DateTime::createFromFormat('Y-m-d H:i:s', $d);
             if (!$dt) return false;
    ...
    if ($base === 'Processos') {
        ...
        $months = $_SESSION['selected_months'] ?? [(int)date('n')];
        $targetFiles = $db->getProcessFiles($years, $months);
        $res = $db->select($targetFiles, $filters, 1, 1000000, 'DATA', true);
        $rows = $res['data'];
    } else {
        if (empty($filters)) {
             $rows = $db->readJSON($db->getPath($base));
        } else {
             $res = $db->select($base, $filters, 1, 1000000); 
             $rows = $res['data'];
        }
    }
...
if (isset($_GET['acao']) && $_GET['acao'] == 'exportar_excel') {
    ...
    ob_clean();
    $filename = "Relatorio_SM_" . date('d-m-Y_His') . ".xls";
    $fBusca = $_GET['fBusca'] ?? '';
    ...
    $checkDate = function($row) use ($extraFilters, $fAtendente, $fStatus, $fDataIni, $fDataFim) {
        ...
        if ($fStatus) {
             $rStat = get_value_ci($row, 'STATUS');
             if (mb_strtoupper(trim((string)$rStat)) !== mb_strtoupper(trim((string)$fStatus))) return false;
        }
    ...
    $hProcSchema = $config->getFields('Base_processos_schema'); 
    $hProcSchema = array_filter($hProcSchema, function($f) { 
        return (($f['type'] ?? '') !== 'title') && !($f['deleted'] ?? false); 
    });
    
    $hProc = [];
    $hProcLabelMap = [];
    foreach ($hProcSchema as $f) {
        $hProc[] = $f['key'];
        $hProcLabelMap[$f['key']] = $f['label'] ?? $f['key'];
    }
    if (!in_array('Ultima_Alteracao', $hProc)) {
        $hProc[] = 'Ultima_Alteracao';
        $hProcLabelMap['Ultima_Alteracao'] = 'Ultima_Alteracao';
    }
    ...
    $ags = [];
    foreach ($processos as $p) {
        $val = get_value_ci($p, 'CPF');
        if($val) $cpfs[] = $val;
        $valAg = get_value_ci($p, 'AG');
        if($valAg) $ags[] = $valAg;
    }
    $cpfs = array_unique($cpfs);
    ...
    // Headers from Config to ensure Dynamic Fields
    $hCli = [];
    $fCli = $config->getFields('Base_clientes.json');
    foreach($fCli as $f) {
        if (($f['type'] ?? '') !== 'title' && !($f['deleted'] ?? false)) $hCli[] = $f['key'];
    }
    if (empty($hCli)) $hCli = $db->getHeaders('Base_clientes.json');

    $agencias = $db->findMany('Base_agencias.json', 'AG', $ags);
    $agenciaMap = []; foreach ($agencias as $a) $agenciaMap[$a['AG']] = $a;
    
    $hAg = [];
    $fAg = $config->getFields('Base_agencias.json');
    foreach($fAg as $f) {
        if (($f['type'] ?? '') !== 'title' && !($f['deleted'] ?? false)) $hAg[] = $f['key'];
    }
    if (empty($hAg)) $hAg = $db->getHeaders('Base_agencias.json');

    $hCred = [];
    $fCred = $config->getFields('Identificacao.json');
    foreach($fCred as $f) {
        if (($f['type'] ?? '') !== 'title' && !($f['deleted'] ?? false)) $hCred[] = $f['key'];
    }
    if (empty($hCred)) $hCred = $db->getHeaders('Identificacao.json');
    ...
    echo "  <Row>\n";
    foreach($hProc as $h) {
        $hLabel = $hProcLabelMap[$h] ?? $h;
        echo "   <Cell ss:StyleID=\"Header\"><Data ss:Type=\"String\">" . htmlspecialchars($hLabel) . "</Data></Cell>\n";
    }
    ...
    // --- Data Aggregation for History/Records ---
    $allEmailHistory = [];
    $allEnvioHistory = [];
    $allProcessRecords = [];

    // 1. Collect from Process Objects (Embedded)
    foreach ($processos as $proc) {
        // Emails / Envios
        if (isset($proc['historico_envios']) && is_array($proc['historico_envios'])) {
            foreach ($proc['historico_envios'] as $h) {
                if (isset($h['TYPE']) && $h['TYPE'] === 'EMAIL') {
                    $allEmailHistory[] = $h;
                } else {
                    $allEnvioHistory[] = $h;
                }
    ...
    // 2. Collect from Legacy Files (if exist)
    $histFile = 'dados/Base_registros.json';
    if (file_exists($histFile)) {
        $legacyRows = json_decode(file_get_contents($histFile), true) ?: [];
    ...
    if (file_exists($regFile)) {
        $legacyRegs = json_decode(file_get_contents($regFile), true) ?: [];
        foreach ($legacyRegs as $row) {
            $pPort = trim(get_value_ci($row, $IDENT_ID_FIELD));
            if (in_array($pPort, $ports)) {
                $allProcessRecords[] = $row;
            }
        }
    }

    // --- SHEET 4: REGISTROS DE ENVIO E-MAILS ---
    echo "<Worksheet ss:Name=\"Registros de Envio E-mails\">\n";
    echo " <Table>\n";
    
    // Headers
    $emailHeaders = ['Data e Hora', $ID_LABEL, 'Responsável', 'Assunto', 'Para', 'CC'];
    echo "  <Row>\n";
    foreach($emailHeaders as $h) echo "   <Cell ss:StyleID=\"Header\"><Data ss:Type=\"String\">$h</Data></Cell>\n";
    echo "  </Row>\n";

    foreach ($allEmailHistory as $row) {
        $valData = $clean(get_value_ci($row, 'DATA'));
        $valPort = $clean(get_value_ci($row, $IDENT_ID_FIELD));
        $valUser = $clean(get_value_ci($row, 'USUARIO'));
        $valAssunto = $clean(get_value_ci($row, 'ASSUNTO'));
        $valPara = $clean(get_value_ci($row, 'PARA'));
        $valCc = $clean(get_value_ci($row, 'CC'));
        ...
        echo "   <Cell><Data ss:Type=\"String\">$valAssunto</Data></Cell>\n";
        echo "   <Cell><Data ss:Type=\"String\">$valPara</Data></Cell>\n";
        echo "   <Cell><Data ss:Type=\"String\">$valCc</Data></Cell>\n";
        echo "  </Row>\n";
    }
    echo " </Table>\n";
    echo "</Worksheet>\n";
    ...
    // Headers
    $histHeaders = ['Data e Hora', $ID_LABEL, 'Usuário', 'Título', 'Tema'];
    echo "  <Row>\n";
    foreach($histHeaders as $h) echo "   <Cell ss:StyleID=\"Header\"><Data ss:Type=\"String\">$h</Data></Cell>\n";
    echo "  </Row>\n";

    foreach ($allEnvioHistory as $row) {
        $valData = $clean(get_value_ci($row, 'DATA'));
        $valPort = $clean(get_value_ci($row, $IDENT_ID_FIELD));
        $valUser = $clean(get_value_ci($row, 'USUARIO'));
        $valMod  = get_value_ci($row, 'MODELO') ?: get_value_ci($row, 'TEXTO');
    ...
    echo "   <Cell ss:StyleID=\"Header\"><Data ss:Type=\"String\">" . htmlspecialchars($ID_LABEL) . "</Data></Cell>\n";
    foreach($regFields as $f) echo "   <Cell ss:StyleID=\"Header\"><Data ss:Type=\"String\">" . htmlspecialchars($f['label']) . "</Data></Cell>\n";
    echo "  </Row>\n";

    foreach ($allProcessRecords as $row) {
        $valData = $clean(get_value_ci($row, 'DATA'));
        $valUser = $clean(get_value_ci($row, 'USUARIO'));
        $valPort = $clean(get_value_ci($row, $IDENT_ID_FIELD));
...
if (isset($_GET['acao']) && $_GET['acao'] == 'exportar_lembretes_excel') {
    $filename = "Lembretes_SM_" . date('d-m-Y_His') . ".xls";
    ...
    if (!in_array('Ultima_Alteracao', $hProcKeys)) {
        array_unshift($hProcKeys, 'Ultima_Alteracao');
        $hProcLabelMap['Ultima_Alteracao'] = 'Ultima_Alteracao';
    }
    if (!in_array('Data_Lembrete', $hProcKeys)) {
        array_unshift($hProcKeys, 'Data_Lembrete');
        $hProcLabelMap['Data_Lembrete'] = 'Data Lembrete';
    }
        ...
        <div class="row">
            ...
            <div class="col-md-12">
                ...
                <ul class="nav nav-tabs mb-3 justify-content-center">
                    ...
                    <li class="nav-item"><button type="button" class="nav-link" data-bs-toggle="tab" data-bs-target="#tab-email">E-mail</button></li>
                ...
                <div class="tab-content">
                    <div class="tab-pane fade show active" id="tab-dados">
                        ...
                        <form method="POST" id="form_processo">
                            <input type="hidden" name="original_id" id="proc_original_id" value="">
                            <input type="hidden" name="timestamp_controle" id="timestamp_controle" value="">
                            <input type="hidden" name="Data_Lembrete" value="">
                            ...
                            foreach($procFields as $f): 
                                ...
                                $val = $getVal($processo, $f['key']);
                                    ...
                                    ?>
                                    <div class="col-md-4 <?= $hideClass ?>" data-field-key="<?= $f['key'] ?>" <?= $isDeleted ? 'data-deleted="true"' : '' ?>>
                                        ...
                                        <?php elseif($f['type'] == 'multiselect'): ?>
                                            ...
                                            ?>
                                            <div class="dropdown">
                                                <button class="btn btn-outline-secondary text-dark fw-bold w-100 text-start dropdown-toggle bg-white form-control-custom" type="button" data-bs-toggle="dropdown" aria-expanded="false" id="btn_ms_<?= $f['key'] ?>">
                    ...
                    <div class="tab-pane fade" id="tab-registros">
                        <input type="hidden" id="reg_uid" name="reg_uid">
                        <div class="card card-custom p-4 mb-4">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <?php elseif($f['type'] == 'multiselect'): ?>
                                        ...
                                        ?>
                                        <div class="dropdown">
                                            <button class="btn btn-outline-secondary text-dark fw-bold w-100 text-start dropdown-toggle bg-white form-control-custom" type="button" data-bs-toggle="dropdown" aria-expanded="false" id="btn_ms_reg_<?= $f['key'] ?>">
                                                <?= htmlspecialchars($btnLabel) ?>
                                            </button>
                                            <ul class="dropdown-menu w-100 p-2" style="max-height: 250px; overflow-y: auto;">
                                            <?php foreach($opts as $opt): 
                                ...
                                <div class="col-12 text-end mt-3">
                                        <button type="button" class="btn btn-primary" id="btn_save_process_record" onclick="saveProcessRecord(this)"><i class="fas fa-save me-1"></i> Salvar Registro</button>
                                </div>
                            </div>
                            <hr class="my-4">
                            ...
                            <div class="table-responsive">
                                <table class="table table-sm table-striped small">
                                    <thead><tr>
                                        <th style="width: 80px;"><input type="checkbox" class="form-check-input" onclick="toggleAllChecks(this, 'registros_del')"></th>
                        ...
                        <div class="card card-custom p-4 mb-4">
                            ...
                            <div class="card mb-3">
                                <div class="card-body">
                                    <div class="row align-items-end">
                                        <div class="col-md-7">
                                            <label class="form-label">Selecionar Arquivo(s)</label>
                                            <input type="file" class="form-control" id="input_anexo_file" multiple>
                                        </div>
                                        <div class="col-md-5 d-flex gap-2">
                                            <button class="btn btn-navy flex-grow-1" onclick="uploadAttachment()"><i class="fas fa-upload me-2"></i>Enviar</button>
                                            <button class="btn btn-navy flex-grow-1" id="btn-download-zip" type="button" onclick="downloadZip()">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-hover" id="table_anexos">
                                    <thead>
                                        <tr>
                                            <th style="width: 40px;"><input type="checkbox" class="form-check-input mb-0" style="margin-top:0;" onclick="toggleAllAttachments(this)"></th>
                                            <th>Arquivo</th>
                    ...
                    <div class="tab-pane fade" id="tab-email">
                        <div class="card card-custom p-4 mb-4">
                            <style>
                                ...
                                .edit-only-email { display: none !important; }
                                .edit-mode-email .edit-only-email { display: block !important; }
                                .edit-mode-email .edit-only-flex-email { display: flex !important; }
                            </style>
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <h5 class="text-navy mb-0"><i class="fas fa-envelope me-2"></i>E-mail</h5>
                                <div class="d-flex align-items-center gap-2">
                                     <div class="edit-only-email edit-only-flex-email align-items-center gap-2"> 
                                         <button class="btn btn-sm btn-outline-primary" onclick="openEmailListModal()"><i class="fas fa-list me-1"></i> Nova Lista</button>
                                         <button class="btn btn-sm btn-outline-secondary" onclick="openEmailTemplateModal()"><i class="fas fa-plus me-1"></i> Novo Modelo</button>
                                         <span class="vr mx-2"></span>
                                     </div>
                                     <button type="button" class="btn btn-sm btn-outline-primary" onclick="openEmailContactModal()"><i class="fas fa-address-book me-1"></i> Gerenciar Contatos</button>
                                     <button class="btn btn-sm btn-outline-warning fw-bold shadow-sm" id="btnToggleEmailEdit" onclick="toggleEmailEditMode()"><i class="fas fa-cog me-1"></i> Gerenciar Temas</button>
                                </div>
                            </div>

                            <div class="row g-3 mb-3">
                                <div class="col-12">
                                    <div id="email_templates_container" class="accordion row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3" style="min-height: 50px;">
                                        <div class="text-center text-muted py-4"><i class="fas fa-spinner fa-spin me-2"></i>Carregando temas...</div>
                                    </div>
                                </div>
                                <div class="col-md-6 position-relative">
                                    <label class="form-label">Para</label>
                                    <div class="input-group">
                                        <input type="text" class="form-control" id="email_to" autocomplete="off" oninput="showEmailAutocomplete(this, 'to')">
                                        <button class="btn btn-outline-secondary" type="button" onclick="openContactSelector('email_to')" title="Selecionar Contatos"><i class="fas fa-search"></i></button>
                                    </div>
                                    <ul id="email_to_autocomplete" class="list-group position-absolute w-100 shadow" style="display:none; z-index: 1050; max-height: 200px; overflow-y: auto; top: 100%;"></ul>
                                </div>
                                <div class="col-md-6 position-relative">
                                    <label class="form-label">CC</label>
                                    <div class="input-group">
                                        <input type="text" class="form-control" id="email_cc" autocomplete="off" oninput="showEmailAutocomplete(this, 'cc')">
                                        <button class="btn btn-outline-secondary" type="button" onclick="openContactSelector('email_cc')" title="Selecionar Contatos"><i class="fas fa-search"></i></button>
                                    </div>
                                    <ul id="email_cc_autocomplete" class="list-group position-absolute w-100 shadow" style="display:none; z-index: 1050; max-height: 200px; overflow-y: auto; top: 100%;"></ul>
                                </div>
                                <div class="col-md-12">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <textarea class="form-control" id="email_body" rows="10"></textarea>
                                </div>
                            </div>

                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <div class="d-flex gap-2">
                                    <button type="button" class="btn btn-primary" onclick="openOutlookWeb()"><i class="fab fa-microsoft me-1"></i> Abrir no Outlook Web</button>
                                    <button type="button" class="btn btn-outline-primary" onclick="openOutlookLocal()"><i class="fas fa-envelope-open-text me-1"></i> Abrir no Outlook Local</button>
                                </div>
                                <div class="d-flex gap-2">
                                    <button type="button" class="btn btn-outline-secondary" onclick="clearEmailSelection()"><i class="fas fa-eraser me-1"></i> Limpar Seleção</button>
                                    <button type="button" class="btn btn-success" onclick="saveEmailHistory(this)"><i class="fas fa-paper-plane me-1"></i> Registrar</button>
                                </div>
                            </div>
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <h5 class="text-navy mb-0">Histórico de Envio de E-mails</h5>
                                <button type="button" class="btn btn-sm btn-outline-danger" onclick="deleteSelectedHistory('email')"><i class="fas fa-trash me-1"></i> Excluir Selecionado</button>
                            </div>
                            
                            <div class="table-responsive">
                                <table class="table table-sm table-striped small">
                                    <thead><tr>
                                        <th style="width: 50px;"><input type="checkbox" class="form-check-input" onclick="toggleAllChecks(this, 'email_hist_del')"></th>
                                        <th>Data/Hora</th>
                                        <th>Responsável</th>
                                        <th>Assunto</th>
                                        <th>Para</th>
                            ...
                            <div class="d-flex justify-content-end align-items-center mb-3">
                                <div class="d-flex align-items-center gap-2">
                                     <div class="edit-only edit-only-flex align-items-center gap-2"> 
                                         ...
                                         <button class="btn btn-sm btn-outline-info" onclick="openGreetingsManager()"><i class="fas fa-hand-spock me-1"></i> Saudações</button>
                                         <span class="vr mx-2"></span>
                                     </div>
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-12">
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <button type="button" class="btn btn-sm btn-link text-decoration-none" onclick="openGreetingSelector()">
                                            <i class="fas fa-hand-spock me-1"></i> Configurar Saudação/Fechamento
                                        </button>
                                    </div>
...
<script>
                 ...
                 if (el.type === 'checkbox' || el.type === 'radio') {
                     if (!el.checked) {
                         if (el.type === 'radio' || el.name.endsWith('[]')) {
                             ...
                             var checked = false;
                             for(var i=0; i<grp.length; i++) { if(grp[i].checked) checked=true; }
    ...
    function reactivateField(file, key) {
        var performReactivation = function() {
            showLoading();
            var fd = new FormData();
            ...
            .then(res => {
                ...
                if(res.status == 'ok') {
                    ...
                    if (container) {
                        ...
                        container.removeAttribute('data-deleted');
        ...
        Swal.fire({
            ...
            if(r.isConfirmed) {
                if (typeof configAuthRequired !== 'undefined' && configAuthRequired && !window.isConfigUnlocked) {
                    Swal.fire({
                        title: 'Acesso Restrito',
                        text: 'Informe a senha de configuração para reativar o campo:',
                        input: 'password',
                        inputAttributes: {
                            autocapitalize: 'off',
                            autocorrect: 'off'
                        ...
                        cancelButtonText: 'Cancelar',
                        preConfirm: (pass) => {
                            var correctPass = typeof CONFIG_PASS !== 'undefined' ? CONFIG_PASS : '1234';
                            if (pass !== correctPass) {
    ...
    function editField(f, file) { 
        ...
        var reqCheck = document.getElementById('field_required');
        reqCheck.checked = (f.required === true || f.required === "true");
        
        // Enforce Fixed Fields
        var fieldKeyUpper = f.key.toUpperCase();
        var isLocked = (file === 'Base_processos_schema') && 
                       (fieldKeyUpper === ID_FIELD_KEY.toUpperCase() || ['DATA', 'STATUS', 'NOME_ATENDENTE'].includes(fieldKeyUpper));
    ...
    function generateText() {
        ...
        if (selectedGreetingId) {
            promises.push(
                fetch('', {
                    method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                    body: 'acao=ajax_generate_greeting_text&id=' + selectedGreetingId + '&data=' + dataJson
                }).then(r => r.json()).then(res => res.status == 'ok' ? res.text : '')
            );
        } else {
        ...
        if(selectedTpls.length > 0) {
            selectedTpls.forEach(tplId => {
                promises.push(
                    fetch('', {
                        method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                        body: 'acao=ajax_generate_text&tpl_id=' + tplId + '&data=' + dataJson
                    }).then(r => r.json()).then(res => res.status == 'ok' ? res.text : '')
                );
            });
        }
        ...
        if (selectedClosingId) {
            promises.push(
                fetch('', {
                    method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                    body: 'acao=ajax_generate_greeting_text&id=' + selectedClosingId + '&data=' + dataJson
                }).then(r => r.json()).then(res => res.status == 'ok' ? res.text : '')
            );
        } else {
            promises.push(Promise.resolve(''));
        ...
        Promise.all(promises).then(results => {
            var fullText = "";
            var greeting = results[0];
            var closing = results[results.length - 1]; // Last one is closing
            var bodyParts = results.slice(1, results.length - 1); // Middle are templates
            
            if (greeting) fullText += greeting + "\n\n";
            if (bodyParts.length > 0) {
                fullText += bodyParts.filter(t => t).join("\n\n") + "\n\n";
            }
            if (closing) fullText += closing;
            
            document.getElementById('tpl_result').value = fullText.trim();
        });
    }
    ...
    function submitProcessForm(btn) {
        ...
        var doSubmit = function() {
            ...
            .then(res => {
                ...
                if (res.status == 'ok') {
                    // Update timestamp on client side so another subsequent save is not blocked
                    if (res.new_timestamp) {
                        var tsInput = form.querySelector('input[name="timestamp_controle"]');
                        if (tsInput) tsInput.value = res.new_timestamp;
                    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Restrict access to specific tabs if no process is loaded
        var restrictedTabs = ['#tab-textos', '#tab-email', '#tab-registros', '#tab-anexos'];
        restrictedTabs.forEach(function(targetId) {
            var triggerEl = document.querySelector('button[data-bs-target="' + targetId + '"]');
            if (triggerEl) {
                triggerEl.addEventListener('show.bs.tab', function(event) {
                    if (!currentLoadedPort) {
                        event.preventDefault();
                        event.stopPropagation();
                        Swal.fire({
                            icon: 'warning',
                            title: 'Atenção',
                            text: 'Selecione um processo para acessar esta aba.'
                        });
                    }
                });
            }
    ...
    function startNewService() {
        ...
        var uidInput = document.getElementById('reg_uid');
        ...
        document.querySelectorAll('#form_processo input, #form_processo select, #form_processo textarea, #form_processo button').forEach(el => {
            if (el.closest('[data-deleted="true"]') && el.tagName !== 'BUTTON') {
                el.disabled = true;
            } else {
                el.disabled = false;
            }
    ...
    function loadProcess(port, btn) {
        ...
        .then(res => {
            ...
            if(res.status == 'ok') {
                ...
                // Set Original ID for renaming detection
                var origIdInput = document.getElementById('proc_original_id');
                if(origIdInput) origIdInput.value = port;
                        ...
                        if(val) {
                            ...
                            if (el.type === 'datetime-local') {
                                if(/^\d{2}\/\d{2}\/\d{4}/.test(val)) {
                                    ...
                                    var timePart = (dtParts.length > 1) ? dtParts[1].substring(0, 5) : '00:00';
                                    if(dParts.length === 3) val = dParts[2] + '-' + dParts[1] + '-' + dParts[0] + 'T' + timePart;
                                }
                            }
                            el.value = val;
                ...
                fields.forEach(f => {
                    ...
                    if(f.type == 'datetime' || f.type == 'datetime-local') {
                        ...
                        if (rawVal && rawVal.includes('/')) {
                             ...
                             var d = parts[0].split('/');
                             var t = (parts.length > 1) ? parts[1].trim() : '00:00';
    ...
    function saveBase() {
        ...
        .then(res => {
            ...
            if(res.status == 'ok') {
                ...
                Swal.fire('Sucesso', res.message, 'success');
                if (typeof window.onBaseSavedHook === 'function') {
                    window.onBaseSavedHook();
                } else {
                    renderBaseTable();
                }
            } else {
        ...
        .then(res => {
            ...
            if(res.status == 'ok') {
                ...
                fields.forEach(f => {
                    ...
                    if (inputType == 'datetime-local' && !disabled && rawVal && rawVal.includes('/')) {
                        var parts = rawVal.split(' ');
                        var d = parts[0].split('/');
                        var t = (parts.length > 1) ? parts[1].trim() : '00:00';
                        if (d.length == 3) safeVal = d[2] + '-' + d[1] + '-' + d[0] + 'T' + t;
                    }
    ...
    function deleteSelectedHistory(type) {
        var name = (type == 'envios') ? 'hist_del' : (type == 'email' ? 'email_hist_del' : 'registros_del');
        var action = (type == 'envios' || type == 'email') ? 'ajax_delete_history' : 'ajax_delete_process_history';
    ...
    function saveEmailHistory(btn) {
        var port = document.getElementById('proc_port').value;
        var to = document.getElementById('email_to').value;
        var cc = document.getElementById('email_cc').value;
        var subject = document.getElementById('email_subject').value;
        var body = document.getElementById('email_body').value;
        
        if(!port) { Swal.fire('Erro', 'Processo não identificado.', 'error'); return; }
        if(!to) { Swal.fire('Erro', 'Destinatário (Para) é obrigatório.', 'error'); return; }
        ...
        var fd = new FormData();
        fd.append('acao', 'ajax_save_history');
        fd.append('port', port);
        fd.append('modelo', 'E-mail: ' + subject);
        fd.append('texto', body);
        fd.append('destinatarios', to + (cc ? '; CC: '+cc : ''));
        ...
        .then(res => {
            setButtonLoading(btn, false);
            if(res.status == 'ok') {
                Swal.fire({title: 'Sucesso', text: 'E-mail registrado no histórico.', icon: 'success', timer: 1500, showConfirmButton: false});
                
                // Fetch only history to avoid full reload
                var fdHist = new FormData();
                fdHist.append('acao', 'ajax_get_process_full');
                fdHist.append('port', port);
                fetch('', { method: 'POST', body: fdHist })
                .then(r2 => r2.text())
                .then(text => {
                    try {
                        var res2 = JSON.parse(text);
                        if(res2.status == 'ok' && res2.email_history) {
                            renderEmailHistory(res2.email_history);
                        }
                    } catch(e) {}
                });
    ...
    function renderEmailHistory(history) {
        var tbody = document.getElementById('email_history_body');
        if(!tbody) return;
        tbody.innerHTML = '';
        ...
        history.forEach(function(h) {
            if (h.TYPE === 'EMAIL') {
                ...
                var idVal = (h._id !== undefined) ? h._id : (h.UID || '');
                // Escape Values
                var safeSubject = (h.ASSUNTO || '').replace(/</g, '&lt;');
                var safeTo = (h.PARA || '').replace(/</g, '&lt;');
                var safeCc = (h.CC || '').replace(/</g, '&lt;');
                
                // For onclick json, we use raw object but be careful with quotes
                var jsonH = JSON.stringify(h).replace(/"/g, '&quot;');

                var editBtn = '<button class="btn btn-sm btn-link text-primary p-0 ms-2" onclick="editEmailHistory('+jsonH+')"><i class="fas fa-pen"></i></button>';
                
                tr.innerHTML = '<td><div class="d-flex align-items-center text-nowrap"><input type="checkbox" name="email_hist_del[]" class="form-check-input mb-0" style="margin-top:0;" value="'+idVal+'">' + editBtn + '</div></td>' + 
                               '<td>'+(h.DATA||'')+'</td><td>'+(h.USUARIO||'')+'</td><td>'+safeSubject+'</td><td>'+safeTo+'</td><td>'+safeCc+'</td>';
                tbody.appendChild(tr);
            }
        });
    }

    function editEmailHistory(h) {
        document.getElementById('email_to').value = h.PARA || '';
        document.getElementById('email_cc').value = h.CC || '';
        document.getElementById('email_subject').value = h.ASSUNTO || '';
        document.getElementById('email_body').value = h.TEXTO || ''; 
        
        var triggerEl = document.querySelector('button[data-bs-target="#tab-email"]');
        if(triggerEl) bootstrap.Tab.getOrCreateInstance(triggerEl).show();
    }
        ...
        .then(resSchema => {
            if(resSchema.status == 'ok') {
                ...
                history.forEach(row => {
                    ...
                    var regId = (row._id !== undefined ? row._id : '');
                    var checkboxHtml = '<input type="checkbox" name="registros_del[]" class="form-check-input" value="'+ regId + '">';
                    
                    var editBtn = document.createElement('button');
                    editBtn.className = 'btn btn-sm btn-link text-primary ms-2 p-0';
                    editBtn.innerHTML = '<i class="fas fa-pen"></i>';
                    editBtn.onclick = function() { editHistoryRecord(row); };
                    
                    tdCheck.innerHTML = checkboxHtml;
    ...
    function editHistoryRecord(row) {
        var uid = (row._id !== undefined) ? row._id : (row.UID || '');
        var uidInput = document.getElementById('reg_uid');
        if(uidInput) uidInput.value = uid;
        
        Object.keys(row).forEach(function(key) {
             var input = document.querySelector('[name="reg_new_'+key+'"]');
             if(!input) {
                 var inputs = document.querySelectorAll('[name="reg_new_'+key+'[]"]');
                 if(inputs.length > 0) {
                     var val = row[key];
                     var values = val ? String(val).split(',').map(s => s.trim()) : [];
                     inputs.forEach(cb => {
                     ...
                     if(inputs[0].classList.contains('ms-checkbox')) {
                         updateMultiselectLabel(inputs[0]);
                     }
                     return;
                 }
             }
             ...
             if(input) {
                 ...
                 if(input.type == 'checkbox' || input.type == 'radio') {
                     ...
                     if (input.type === 'date' && val && val.includes('/')) {
                        var parts = val.split('/');
                        if (parts.length == 3) val = parts[2] + '-' + parts[1] + '-' + parts[0];
                     }
                     if (input.type === 'datetime-local' && val && val.includes('/')) {
                        var parts = val.split(' ');
                        var d = parts[0].split('/');
                        var t = parts[1] || '00:00';
                        if (d.length == 3) val = d[2] + '-' + d[1] + '-' + d[0] + 'T' + t;
                     }
                     input.value = val;
                 }
             }
        ...
        var triggerEl = document.querySelector('button[data-bs-target="#tab-registros"]');
        if(triggerEl) bootstrap.Tab.getOrCreateInstance(triggerEl).show();
        
        var card = document.querySelector('#tab-registros .card-custom');
        if(card) card.scrollIntoView({behavior: 'smooth'});
        
        Swal.fire({
            icon: 'info',
            title: 'Editando Registro',
            text: 'Faça as alterações e clique em Salvar.',
            timer: 2000,
            showConfirmButton: false
        });
        
        var btnSave = document.getElementById('btn_save_process_record');
        if(btnSave) btnSave.innerHTML = '<i class="fas fa-sync-alt me-1"></i> Atualizar Registro';
    }
    ...
    function saveProcessRecord(btn) {
        ...
        var uidInput = document.getElementById('reg_uid');
        if(uidInput && uidInput.value) formData.append('uid', uidInput.value);
        ...
        .then(res => {
            ...
            if(res.status == 'ok') {
                ...
                document.querySelectorAll('.reg-new-field').forEach(el => {
                    if(el.type === 'checkbox' || el.type === 'radio') {
                        el.checked = false;
                        if(el.classList.contains('ms-checkbox')) updateMultiselectLabel(el);
                    } else {
                        el.value = '';
                    }
                });
                ...
                // Reset Button
                var btnSave = document.getElementById('btn_save_process_record');
                if(btnSave) btnSave.innerHTML = '<i class="fas fa-save me-1"></i> Salvar Registro';
            } else {
                Swal.fire('Erro', res.message || 'Erro ao salvar', 'error');
            }
        })
...
<script>
    ...
    function uploadAttachment() {
        ...
        if(fileInput.files.length === 0) {
            Swal.fire('Erro', 'Selecione pelo menos um arquivo.', 'error');
            return;
        }
            ...
            if(tbody) {
                ...
                if(res.status == 'ok' && res.files) {
                    if(res.files.length === 0) {
                        tbody.innerHTML = '<tr><td colspan="4" class="text-center text-muted">Nenhum arquivo anexado.</td></tr>';
                    } else {
                        res.files.forEach(f => {
                            var tr = document.createElement('tr');
                            tr.innerHTML = `
                                <td><input type="checkbox" class="form-check-input check-anexo mb-0" style="margin-top:0;" value="${f.name}"></td>
    ...
    function toggleAllAttachments(source) {
        document.querySelectorAll('.check-anexo').forEach(cb => cb.checked = source.checked);
    }
    
    function downloadZip() {
        var files = [];
        var checkboxes = document.querySelectorAll('.check-anexo');
        
        if (checkboxes.length === 0) {
            Swal.fire('Aviso', 'Não há anexos para baixar.', 'info');
            return;
        }
        
        var checkedBoxes = document.querySelectorAll('.check-anexo:checked');
        
        if (checkedBoxes.length > 0 && checkedBoxes.length < checkboxes.length) {
            checkedBoxes.forEach(cb => files.push(cb.value));
        }
        
        var fd = new FormData();
        fd.append('acao', 'ajax_download_zip');
        fd.append('port', currentLoadedPort);
        ...
        var btn = document.querySelector('#btn-download-zip');
        var originalText = '';
        if (btn) {
        ...
        .then(r => {
            ...
            if (r.headers.get('content-type') && r.headers.get('content-type').indexOf('application/json') !== -1) {
                return r.json().then(data => { throw new Error(data.message || 'Erro desconhecido'); });
            }
            return r.blob();
        })
        .then(blob => {
            ...
            var a = document.createElement('a');
            a.href = url;
            a.download = `Anexos_${currentLoadedPort}.zip`;
            document.body.appendChild(a);
            a.click();
            a.remove();
            window.URL.revokeObjectURL(url);
        ...
        .catch(err => {
            ...
            Swal.fire('Erro', err.message || 'Falha na comunicação', 'error');
        });
    }
...
<!-- Modal Subject Selector -->
<div class="modal fade" id="modalSubjectSelector" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header bg-navy text-white">
                <h5 class="modal-title">Selecionar Tema</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body bg-light">
                <div class="d-flex justify-content-end mb-2">
                     <button class="btn btn-sm btn-outline-primary" onclick="openTemplateModal()"><i class="fas fa-plus"></i> Novo Tema</button>
                </div>
                <div id="subject_selector_container" class="accordion accordion-flush"></div>
            </div>
        </div>
    </div>
</div>
...
let allLists = [];
let userOrder = [];
var modalList, modalTemplate, modalSubjectSelector;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    modalSubjectSelector = new bootstrap.Modal(document.getElementById('modalSubjectSelector'));
    loadTemplatesData();
});
        ...
        if (t && contentDiv) {
            ...
            document.querySelectorAll('#form_processo input, #form_processo select, #form_processo textarea').forEach(function(el) {
                if(el.name) {
                    ...
                    // Format ISO Date (Y-m-d) -> d/m/Y
                    if (/^\d{4}-\d{2}-\d{2}$/.test(val)) {
                        var p = val.split('-');
                        val = p[2] + '/' + p[1] + '/' + p[0];
                    }
                    // Format ISO Datetime (Y-m-dTH:i) -> d/m/Y H:i
...
function openSubjectSelector() {
    renderSubjectSelectorUI();
    modalSubjectSelector.show();
}

function renderSubjectSelectorUI() {
    const container = document.getElementById('subject_selector_container');
    container.innerHTML = '';
    ...
    sortedLists.forEach((list) => {
        container.appendChild(createSubjectSelectorList(list, null));
    });
    ...
    if(uncategorized.length > 0) {
        let fakeList = {id: 'uncategorized', name: 'Sem Categoria (Geral)'};
        container.appendChild(createSubjectSelectorList(fakeList, uncategorized));
    }
}

function createSubjectSelectorList(list, forcedTemplates) {
    let tpls = forcedTemplates || allTemplates.filter(t => t.list_id == list.id);
    let item = document.createElement('div');
    item.className = 'accordion-item border rounded mb-2 shadow-sm';
    
    let headerId = 'sel_heading_' + list.id;
    let collapseId = 'sel_collapse_' + list.id;
    
    let html = `
        <h2 class="accordion-header" id="${headerId}">
            <button class="accordion-button collapsed fw-bold text-navy" type="button" data-bs-toggle="collapse" data-bs-target="#${collapseId}">
                ${list.name}
            </button>
        </h2>
        <div id="${collapseId}" class="accordion-collapse collapse" data-bs-parent="#subject_selector_container">
            <div class="accordion-body p-0">
                <ul class="list-group list-group-flush">
                    ${tpls.map(t => createSubjectSelectorItem(t)).join('')}
                </ul>
            </div>
        </div>
    `;
...
function createSubjectSelectorItem(t) {
    // Escape for safety
    let safeTitle = (t.titulo || '').replace(/"/g, '&quot;');
    return `
    <li class="list-group-item list-group-item-action d-flex justify-content-between align-items-center" onclick="selectSubject('${t.id}')" style="cursor:pointer">
        <div>
            <div class="fw-bold">${t.titulo}</div>
            <div class="small text-muted text-truncate" style="max-width: 300px;">${(t.corpo || '').substring(0, 50)}...</div>
        </div>
        <div>
            <button class="btn btn-sm btn-outline-secondary me-1" onclick="event.stopPropagation(); openTemplateModal('${t.id}')"><i class="fas fa-pen"></i></button>
...
function selectSubject(id) {
    ...
    if(tpl) {
        document.getElementById('email_subject_display').value = tpl.titulo;
        document.getElementById('email_template_select').value = id;
    }
    modalSubjectSelector.hide();
}
...
<div class="modal fade" id="modalEmailContacts" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-navy text-white">
                <h5 class="modal-title"><i class="fas fa-users me-2"></i>Selecionar Contatos</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <div class="alert alert-info small mb-3">
                    <i class="fas fa-info-circle me-1"></i> Selecione os destinatários para o envio.
                </div>
                <div class="table-responsive">
                    <table class="table table-sm table-hover">
                        <thead>
                            <tr>
                                <th style="width: 40px;"><input type="checkbox" class="form-check-input" onclick="toggleAllEmailContacts(this)"></th>
                                <th>Nome</th>
                                <th>E-mail</th>
                            </tr>
                        </thead>
                        <tbody id="email_contacts_list">
                            <!-- Populated by JS -->
                        </tbody>
                    </table>
                </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                <button type="button" class="btn btn-navy" onclick="applyEmailContacts()">Confirmar</button>
            </div>
        </div>
    </div>
</div>
...
<script>
var modalEmailContacts = new bootstrap.Modal(document.getElementById('modalEmailContacts'));
var currentEmailTarget = '';

function openContactSelector(targetId) {
    currentEmailTarget = targetId;
    var tbody = document.getElementById('email_contacts_list');
    tbody.innerHTML = '<tr><td colspan="3" class="text-center"><div class="spinner-border spinner-border-sm"></div> Carregando...</td></tr>';
    
    // 1. Fetch from Base
    var fd = new FormData();
    fd.append('acao', 'ajax_get_base_data_json');
    fd.append('base', 'Base_contatos_email.json');
    ...
    .then(res => {
        ...
        if (res.status == 'ok' && Array.isArray(res.data)) {
            contacts = res.data.map(c => ({
                name: c['Nome'] || '',
                email: c['E-mail'] || c['Email'] || c['email'] || ''
            }));
        }
        
        // 2. Fetch from Process (Client/Agency)
        document.querySelectorAll('#form_processo input').forEach(el => {
            var name = el.name.toLowerCase();
            if ((name.includes('client_') || name.includes('agency_')) && (name.includes('mail') || name.includes('email'))) {
                if (el.value) {
                    el.value.split(/[;,]/).forEach(e => { 
                        var clean = e.trim();
                        if(clean.includes('@')) {
                            contacts.push({name: 'Processo/Agência', email: clean});
                        }
                    });
                }
            }
        ...
        var unique = {};
        contacts.forEach(c => {
            if(c.email) unique[c.email] = c.name;
        });
        
        // Sort
        var sortedEmails = Object.keys(unique).sort(function (a, b) {
            return unique[a].localeCompare(unique[b]);
        });
        
        if (sortedEmails.length === 0) {
            tbody.innerHTML = '<tr><td colspan="3" class="text-center text-muted">Nenhum contato encontrado.</td></tr>';
        } else {
            sortedEmails.forEach(email => {
                var name = unique[email];
                var tr = document.createElement('tr');
                tr.innerHTML = `
                    <td><input type="checkbox" class="form-check-input chk-contact-select" value="${email}"></td>
                    <td>${name}</td>
                    <td>${email}</td>
                `;
                tbody.appendChild(tr);
        ...
        modalEmailContacts.show();
    })
    .catch(() => {
        tbody.innerHTML = '<tr><td colspan="3" class="text-center text-danger">Erro ao carregar contatos.</td></tr>';
    });
}

function toggleAllEmailContacts(source) {
    document.querySelectorAll('.chk-contact-select').forEach(cb => cb.checked = source.checked);
}

function applyEmailContacts() {
    var selected = [];
    document.querySelectorAll('.chk-contact-select:checked').forEach(cb => selected.push(cb.value));
    
    if (currentEmailTarget) {
        var input = document.getElementById(currentEmailTarget);
        var currentVal = input.value;
        var parts = currentVal.split(';').map(s => s.trim()).filter(s => s);
    ...
    modalEmailContacts.hide();
}

function renderEmailTemplatesUI() {
    var select = document.getElementById('email_template_select');
    if(!select) return;
    ...
    allTemplates.forEach(t => {
        var lid = t.list_id || 'uncategorized';
    ...
    var sortedListIds = [];
    if(Array.isArray(userOrder)) {
        userOrder.forEach(id => { if(listMap[id]) sortedListIds.push(id); });
    }
    Object.keys(listMap).forEach(id => { if(!sortedListIds.includes(id)) sortedListIds.push(id); });
    if(grouped['uncategorized']) sortedListIds.push('uncategorized');
    
    sortedListIds.forEach(lid => {
        if(grouped[lid]) {
            var groupName = (lid === 'uncategorized') ? 'Sem Categoria' : listMap[lid].name;
            var optgroup = document.createElement('optgroup');
            optgroup.label = groupName;
            
            grouped[lid].forEach(t => {
...
function selectEmailTemplate(id) {
    if(!id) {
        ...
        var disp = document.getElementById('email_subject_display');
        if(disp) disp.value = '';
        return;
    }
    ...
    if(tpl) {
        ...
        document.querySelectorAll('#form_processo input, #form_processo select, #form_processo textarea').forEach(function(el) {
            if(el.name) {
                ...
                if (/^\d{4}-\d{2}-\d{2}$/.test(val)) val = val.split('-').reverse().join('/');
                data[k] = val;
            }
        });
        
        // Dynamic Keys Injection
        var procPort = document.getElementById('proc_port');
        if (procPort) {
            data['NúmeroProcesso'] = procPort.value;
            data['NumeroProcesso'] = procPort.value; 
        }
        ...
        // Fill Subject (Title with replacement) and Body
        var subject = tpl.titulo;
        var body = tpl.corpo;
        ...
        var replace = function(text) {
            ...
            return text.replace(/\{([^}]+)\}/g, function(match, key) {
                var upperKey = key.trim().toUpperCase();
                for (var dk in data) {
                    if (dk.toUpperCase() === upperKey && data[dk]) return data[dk];
                }
                return match;
            });
        };
        
        document.getElementById('email_subject').value = replace(subject);
        document.getElementById('email_body').value = replace(body);
        
        var disp = document.getElementById('email_subject_display');
        if(disp) disp.value = tpl.titulo;
    }
}
...
function openOutlookLocal() {
    var to = document.getElementById('email_to').value;
    var cc = document.getElementById('email_cc').value;
    var subject = document.getElementById('email_subject').value;
    var body = document.getElementById('email_body').value;
    
    // mailto body handling is basic
...
function clearEmailSelection() {
    var radios = document.querySelectorAll('.email-tpl-radio');
...
function saveLembrete() {
    ...
    var form = document.querySelector('#modalLembrete form');
    var btn = form.querySelector('.btn-navy');
    ...
    var fd = new FormData();
    fd.append('acao', 'ajax_save_lembrete');
    fd.append('id', pid);
    fd.append('data_lembrete', val);
    fd.append('ultima_alteracao', lastUpdateVal);
    ...
    .then(res => {
        if(res.status == 'ok') {
            ...
            // Security Lock Correction
            var tsControl = document.getElementById('timestamp_controle');
            if(tsControl && res.ultima_alteracao) {
                tsControl.value = res.ultima_alteracao;
            }
    ...
    if (val && val.trim() !== '') {
        ...
        if (val.indexOf('/') > -1) {
            ...
            if (parts.length >= 2) {
                 ...
                 if (dParts.length === 3) {
                     lDate = new Date(dParts[2], dParts[1]-1, dParts[0], tParts[0], tParts[1]);
                 }
            }
        } 
        // Try ISO Y-m-dTH:i or Y-m-d H:i
        ...
        if (lDate && !isNaN(lDate.getTime())) {
             ...
             var d = String(lDate.getDate()).padStart(2, '0');
             var m = String(lDate.getMonth() + 1).padStart(2, '0');
             var y = lDate.getFullYear();
             var h = String(lDate.getHours()).padStart(2, '0');
             var min = String(lDate.getMinutes()).padStart(2, '0');
             var formattedVal = d + '/' + m + '/' + y + ' ' + h + ':' + min;

             if (lDate < now) {
                 colorClass = 'text-danger';
                 isBeat = true;
                 title = 'Lembrete Vencido: ' + formattedVal;
             } else {
                 colorClass = 'text-success';
                 title = 'Lembrete: ' + formattedVal;
             }
        }
    }
...
<!-- Modal Manage Email List -->
<div class="modal fade" id="modalEmailManageList" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-navy text-white">
                <h5 class="modal-title">Gerenciar Lista de E-mail</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                ...
                <div class="mb-3">
                    <label class="form-label">Nome da Lista</label>
                    <input type="text" class="form-control" id="email_list_name">
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                <button type="button" class="btn btn-primary" onclick="saveEmailList()">Salvar</button>
            </div>
        </div>
    </div>
</div>

<!-- Modal Manage Email Template -->
<div class="modal fade" id="modalEmailManageTemplate" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-navy text-white">
                <h5 class="modal-title">Gerenciar Modelo de E-mail</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <input type="hidden" id="email_tpl_id">
                <div class="mb-3">
                    <label class="form-label">Assunto</label>
                    <input type="text" class="form-control" id="email_tpl_title">
                </div>
                <div class="mb-3">
                    <label class="form-label">Lista</label>
                    <select class="form-select" id="email_tpl_list_select">
                        <!-- Populated JS -->
                    </select>
                </div>
                <div class="mb-3">
                    <label class="form-label">Corpo do E-mail</label>
                    <textarea class="form-control" id="email_tpl_body" rows="10"></textarea>
                    <div class="form-text">Variáveis disponíveis: {Nome}, {ID}, etc.</div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                <button type="button" class="btn btn-primary" onclick="saveEmailTemplate()">Salvar</button>
            </div>
        </div>
    </div>
</div>
<!-- Modal Gerenciar Contatos -->
<div class="modal fade" id="modalEmailContactsManager" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header bg-navy text-white">
                <h5 class="modal-title"><i class="fas fa-address-book me-2"></i>Gerenciar Contatos de E-mail</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-0">
                <div id="email_contact_form" class="bg-light p-3 border-bottom" style="display:none;">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <h6 id="email_contact_form_title" class="m-0 text-navy fw-bold">Novo Contato</h6>
                        <button type="button" class="btn-close btn-sm" onclick="hideEmailContactForm()"></button>
                    </div>
                    <form id="form_email_contact_inline" onsubmit="event.preventDefault(); saveEmailContactInline();">
                        <input type="hidden" id="ecm_id" name="original_id">
                        <input type="hidden" name="base" value="Base_contatos_email.json">
                        <input type="hidden" name="acao" value="ajax_save_base_record">
                        <div class="row g-2">
                            <div class="col-md-5">
                                <label class="form-label small mb-1">Nome</label>
                                <input type="text" class="form-control form-control-sm" name="Nome" id="ecm_nome" required>
                            </div>
                            <div class="col-md-5">
                                <label class="form-label small mb-1">E-mail(s) (separados por vírgula)</label>
                                <input type="text" class="form-control form-control-sm" name="E-mail" id="ecm_email" required>
                            </div>
                            <div class="col-md-2 d-flex align-items-end">
                                <button type="submit" class="btn btn-sm btn-success w-100" id="btn_save_ecm"><i class="fas fa-check me-1"></i> Salvar</button>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <div class="p-3">
                    <div class="d-flex justify-content-between mb-3 border-bottom pb-3">
                        <input type="text" id="email_contact_search" class="form-control w-50" placeholder="Buscar contato..." oninput="filterEmailContactsManager()">
                        <button class="btn btn-primary" onclick="showEmailContactForm(null)"><i class="fas fa-plus me-1"></i> Novo Contato</button>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-sm table-striped table-hover mb-0">
                            ...
                            <tbody id="email_contacts_manager_body">
                                <!-- Populated JS -->
                            </tbody>
                        </table>
                    </div>
...
let allEmailLists = [];
let userEmailOrder = [];
var modalEmailList, modalEmailTemplate, modalEmailContactsManager;
var isEmailEditMode = false;

document.addEventListener('DOMContentLoaded', function() {
    modalEmailList = new bootstrap.Modal(document.getElementById('modalEmailManageList'));
    modalEmailTemplate = new bootstrap.Modal(document.getElementById('modalEmailManageTemplate'));
    modalEmailContactsManager = new bootstrap.Modal(document.getElementById('modalEmailContactsManager'));
    
    // Pre-load email contacts for autocomplete
    var fd = new FormData();
    fd.append('acao', 'ajax_get_base_data_json');
    fd.append('base', 'Base_contatos_email.json');
    fetch('', { method: 'POST', body: fd }).then(r=>r.json()).then(res => {
        if(res.status == 'ok') window.emailContactsData = res.data || [];
    });
});

document.addEventListener('click', function(e) {
    if(!e.target.closest('#email_to') && !e.target.closest('#email_to_autocomplete')) {
        let ul1 = document.getElementById('email_to_autocomplete');
        if(ul1) ul1.style.display = 'none';
    }
    if(!e.target.closest('#email_cc') && !e.target.closest('#email_cc_autocomplete')) {
        let ul2 = document.getElementById('email_cc_autocomplete');
        if(ul2) ul2.style.display = 'none';
    }
});

function showEmailAutocomplete(inputEl, type) {
    var ulId = type === 'to' ? 'email_to_autocomplete' : 'email_cc_autocomplete';
    var ul = document.getElementById(ulId);
    if(!ul) return;
    
    var val = inputEl.value;
    var parts = val.split(/[,;]/);
    var currentTerm = parts[parts.length - 1].trim().toLowerCase();
    ...
    var matches = (window.emailContactsData || []).filter(c => {
        var nome = (c.Nome || '').toLowerCase();
        var email = (c['E-mail'] || '').toLowerCase();
        return nome.includes(currentTerm) || email.includes(currentTerm);
    });
    ...
    ul.innerHTML = '';
    matches.forEach(c => {
        var emails = (c['E-mail']||'').split(',').map(e => e.trim()).filter(e=>e);
        emails.forEach(emailStr => {
            var li = document.createElement('li');
            li.className = 'list-group-item list-group-item-action cursor-pointer px-3 py-2 border-bottom';
            li.innerHTML = `<div class="fw-bold" style="font-size:0.9rem">${c.Nome}</div><div class="small text-muted">${emailStr}</div>`;
            li.onclick = function() {
...
function openEmailContactModal() { 
    hideEmailContactForm();
    var tbody = document.getElementById('email_contacts_manager_body');
    tbody.innerHTML = '<tr><td colspan="3" class="text-center py-4"><i class="fas fa-spinner fa-spin me-2"></i>Carregando...</td></tr>';
    ...
    .then(res => {
        tbody.innerHTML = '';
        window.emailContactsData = [];
        if(res.status == 'ok') {
            window.emailContactsData = res.data || [];
            renderEmailContactsManager();
        } else {
            tbody.innerHTML = '<tr><td colspan="3" class="text-center text-danger">Erro ao carregar dados.</td></tr>';
        }
    }).catch(() => {
        tbody.innerHTML = '<tr><td colspan="3" class="text-center text-danger">Falha de comunicação.</td></tr>';
    });
    
    modalEmailContactsManager.show();
}

function showEmailContactForm(c) {
    document.getElementById('email_contact_form').style.display = 'block';
    if(c) {
        document.getElementById('email_contact_form_title').innerText = 'Editar Contato';
        document.getElementById('ecm_id').value = c._id || c.UID || c.Nome || '';
        document.getElementById('ecm_nome').value = c.Nome || '';
        document.getElementById('ecm_email').value = c['E-mail'] || '';
    } else {
        document.getElementById('email_contact_form_title').innerText = 'Novo Contato';
        document.getElementById('ecm_id').value = '';
        document.getElementById('ecm_nome').value = '';
        document.getElementById('ecm_email').value = '';
    }
    document.getElementById('ecm_nome').focus();
}

function hideEmailContactForm() {
    document.getElementById('email_contact_form').style.display = 'none';
}

function saveEmailContactInline() {
    var form = document.getElementById('form_email_contact_inline');
    if(!form.checkValidity()) { form.reportValidity(); return; }
    
    var btn = document.getElementById('btn_save_ecm');
    setButtonLoading(btn, true);
    ...
    .then(res => {
        ...
        if(res.status == 'ok') {
            hideEmailContactForm();
            openEmailContactModal(); // reload list seamlessly
            Swal.fire({title: 'Salvo!', icon: 'success', toast: true, position: 'top-end', showConfirmButton: false, timer: 1500});
        } else {
            Swal.fire('Erro', res.message, 'error');
        }
    }).catch(() => {
...
function renderEmailContactsManager() {
    var tbody = document.getElementById('email_contacts_manager_body');
    var term = (document.getElementById('email_contact_search').value || '').toLowerCase();
    
    tbody.innerHTML = '';
    
    // Dedup and filter
    var filtered = window.emailContactsData.filter(c => {
        var nome = (c.Nome || '').toLowerCase();
        var email = (c['E-mail'] || '').toLowerCase();
    ...
    filtered.forEach(c => {
         var tr = document.createElement('tr');
         var safeC = JSON.stringify(c).replace(/"/g, '&quot;');
         var idVal = c._id || c.UID || c.Nome;
         
         tr.innerHTML = `
             <td class="align-middle">${c.Nome || ''}</td>
             <td class="align-middle">${c['E-mail'] || ''}</td>
             <td class="text-nowrap align-middle">
                 <button class="btn btn-sm btn-link text-primary p-0 me-2" onclick="showEmailContactForm(${safeC})" title="Editar"><i class="fas fa-pen"></i></button>
                 <button class="btn btn-sm btn-link text-danger p-0" onclick="deleteEmailContactManager('${idVal}')" title="Excluir"><i class="fas fa-trash"></i></button>
             </td>
         `;
         tbody.appendChild(tr);
    });
...
function filterEmailContactsManager() {
    renderEmailContactsManager();
}

function deleteEmailContactManager(id) {
    Swal.fire({
        ...
        if(r.isConfirmed) {
            Swal.fire({title: 'Aguarde', allowOutsideClick: false});
            Swal.showLoading();
            var fd = new FormData();
            fd.append('acao', 'ajax_delete_base_bulk');
            fd.append('base', 'Base_contatos_email.json');
            ...
            .then(res => {
                if(res.status == 'ok') {
                    openEmailContactModal(); // reload silently
                    Swal.fire({title: 'Excluído!', icon: 'success', toast: true, position: 'top-end', showConfirmButton: false, timer: 1500});
...
function loadEmailTemplatesData() {
    fetch('', {
        method: 'POST', 
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: 'acao=ajax_get_email_templates_data'
    })
    .then(r => r.json())
    .then(res => {
        if(res.status == 'ok') {
            allEmailTemplates = res.data.templates;
            allEmailLists = res.data.lists;
            userEmailOrder = res.data.order;
            renderEmailTemplatesUINew();
        }
    });
}

// Global email list reload trigger
document.addEventListener('DOMContentLoaded', function() {
...
function toggleEmailEditMode() {
    ...
    let container = document.getElementById('tab-email'); 
    let btn = document.getElementById('btnToggleEmailEdit');
    
    if(isEmailEditMode) {
        container.classList.add('edit-mode-email');
        btn.innerHTML = '<i class="fas fa-check me-1"></i> Concluir Edição';
        btn.classList.remove('btn-outline-warning');
        btn.classList.add('btn-success');
    } else {
        container.classList.remove('edit-mode-email');
        btn.innerHTML = '<i class="fas fa-cog me-1"></i> Gerenciar Temas';
        btn.classList.remove('btn-success');
        btn.classList.add('btn-outline-warning');
    }
    renderEmailTemplatesUINew();
...
function renderEmailTemplatesUINew() {
    const container = document.getElementById('email_templates_container');
    if(!container) return;
    container.innerHTML = '';
    
    let listMap = {};
    allEmailLists.forEach(l => listMap[l.id] = l);
    
    let sortedLists = [];
    if(Array.isArray(userEmailOrder)) {
        userEmailOrder.forEach(lid => {
            if(listMap[lid]) {
                sortedLists.push(listMap[lid]);
                delete listMap[lid];
            }
    ...
    sortedLists.forEach((list) => {
        container.appendChild(createEmailListElement(list, null));
    });
    
    let uncategorized = allEmailTemplates.filter(t => !t.list_id || t.list_id === '');
    if(uncategorized.length > 0) {
        let fakeList = {id: 'uncategorized', name: 'Sem Categoria (Geral)'};
        container.appendChild(createEmailListElement(fakeList, uncategorized));
    }
}

function handleEmailDragStart(e) {
    if(!isEmailEditMode) { e.preventDefault(); return; }
    e.currentTarget.classList.add('dragging');
    e.dataTransfer.effectAllowed = 'move';
    e.dataTransfer.setData('text/plain', e.currentTarget.dataset.id);
}
function handleEmailDragOver(e) {
    e.preventDefault();
    e.dataTransfer.dropEffect = 'move';
    return false;
}
function handleEmailDragEnter(e) {
    e.currentTarget.classList.add('drag-over');
}
function handleEmailDragLeave(e) {
...
function handleEmailDrop(e) {
    ...
    if(draggedId && draggedId !== targetId) {
        let draggedEl = document.querySelector(`.accordion-item[data-id="${draggedId}"]`);
        let targetEl = e.currentTarget;
        let container = draggedEl.parentNode;
        
        let allItems = Array.from(container.querySelectorAll('.accordion-item:not([data-id="uncategorized"])'));
        let draggedIndex = allItems.indexOf(draggedEl);
        let targetIndex = allItems.indexOf(targetEl);
        ...
        let newOrder = Array.from(container.querySelectorAll('.accordion-item[data-id]'))
                            .map(el => el.dataset.id)
                            .filter(id => id && id !== 'uncategorized');
        
        var fd = new FormData();
        fd.append('acao', 'ajax_reorder_email_lists');
        newOrder.forEach(id => fd.append('order[]', id));
        fetch('', {method: 'POST', body: fd}).then(() => loadEmailTemplatesData());
    }
    return false;
}
function handleEmailDragEnd(e) {
    e.currentTarget.classList.remove('dragging');
    document.querySelectorAll('.drag-over').forEach(el => el.classList.remove('drag-over'));
}

function createEmailListElement(list, forcedTemplates) {
    let tpls = forcedTemplates || allEmailTemplates.filter(t => t.list_id == list.id);
    let item = document.createElement('div');
    item.className = 'accordion-item border rounded h-100 shadow-sm'; 
    
    if(list.id !== 'uncategorized' && isEmailEditMode) {
        item.setAttribute('draggable', 'true');
        item.dataset.id = list.id;
        item.addEventListener('dragstart', handleEmailDragStart);
        item.addEventListener('dragover', handleEmailDragOver);
        item.addEventListener('drop', handleEmailDrop);
        item.addEventListener('dragenter', handleEmailDragEnter);
        item.addEventListener('dragleave', handleEmailDragLeave);
        item.addEventListener('dragend', handleEmailDragEnd);
    }
    
    let headerId = 'email_heading_' + list.id;
    let collapseId = 'email_collapse_' + list.id;
    
    let editBtn = (list.id !== 'uncategorized') 
        ? `<div class="ms-auto edit-only-email" style="z-index:10;">
            <button class="btn btn-link btn-sm text-white-50 p-0 me-2" onclick="openEditEmailList('${list.id}', '${list.name}', event)" title="Editar Lista"><i class="fas fa-edit"></i></button>
            <button class="btn btn-link btn-sm text-danger p-0" onclick="deleteEmailList('${list.id}', event)" title="Excluir Lista"><i class="fas fa-trash"></i></button>
        </div>` 
        : '';
    ...
    let html = `
        ...
        <div id="${collapseId}" class="accordion-collapse collapse" aria-labelledby="${headerId}" data-bs-parent="#email_templates_container">
            <div class="accordion-body bg-white p-2" style="max-height: 300px; overflow-y: auto;">
                <ul class="list-group list-group-flush">
                    ${tpls.map(t => createEmailItemElement(t)).join('')}
                    ${tpls.length === 0 ? '<li class="list-group-item text-muted small text-center">Nenhum modelo</li>' : ''}
                </ul>
                ${list.id !== 'uncategorized' ? 
                `<div class="text-center mt-2 pt-2 border-top edit-only-email">
                    <button class="btn btn-sm btn-outline-primary rounded-pill px-3" onclick="openEmailTemplateModal(null, '${list.id}')"><i class="fas fa-plus me-1"></i> Novo Modelo</button>
                </div>` : ''}
            </div>
        </div>
    `;
...
function createEmailItemElement(t) {
    let safeCorpo = (t.corpo || '').replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/'/g,'&#39;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    return `
    <li class="list-group-item bg-transparent border-bottom p-0">
        <div class="d-flex justify-content-between align-items-center px-3 py-2" style="cursor:pointer" onclick="selectEmailTemplateNew('${t.id}')">
            <label class="form-check-label d-flex align-items-center flex-grow-1 cursor-pointer" style="min-width:0;">
                <div class="text-truncate" style="min-width:0;">
                    <div class="fw-bold text-navy text-truncate" style="font-size:0.9rem">${t.titulo}</div>
                    <div class="small text-muted text-truncate" style="font-size:0.8rem" title="${safeCorpo}">${(t.corpo || '').substring(0, 60)}...</div>
                </div>
            </label>
            <div class="edit-only-email ms-2 text-nowrap flex-shrink-0">
                <button class="btn btn-sm btn-outline-secondary py-0 px-2 border-0" onclick="event.stopPropagation(); openEmailTemplateModal('${t.id}')" title="Editar"><i class="fas fa-pen small"></i></button>
                <button class="btn btn-sm btn-outline-danger py-0 px-2 border-0" onclick="event.stopPropagation(); deleteEmailTemplate('${t.id}')" title="Excluir"><i class="fas fa-trash small"></i></button>
            </div>
        </div>
    </li>
    `;
...
function openEmailListModal() {
    document.getElementById('email_list_id').value = '';
    document.getElementById('email_list_name').value = '';
    modalEmailList.show();
}

function openEditEmailList(id, name, event) {
    if(event) event.stopPropagation();
    document.getElementById('email_list_id').value = id;
    document.getElementById('email_list_name').value = name;
    modalEmailList.show();
}

function saveEmailList() {
    var id = document.getElementById('email_list_id').value;
    var name = document.getElementById('email_list_name').value;
    var fd = new FormData();
    fd.append('acao', 'ajax_save_email_list');
    fd.append('id', id);
    fd.append('name', name);
    fetch('', { method: 'POST', body: fd })
    .then(r => r.json())
...
function deleteEmailList(id, event) {
    if(event) event.stopPropagation();
    Swal.fire({
        title: 'Excluir Lista?',
        text: 'Os modelos desta lista irão para Sem Categoria.',
        icon: 'warning',
        showCancelButton: true
    }).then((result) => {
        if (result.isConfirmed) {
            var fd = new FormData();
            fd.append('acao', 'ajax_delete_email_list');
            fd.append('id', id);
            fetch('', { method: 'POST', body: fd })
            .then(r => r.json())
            .then(res => {
                if(res.status == 'ok') loadEmailTemplatesData();
            });
        }
    });
}

function openEmailTemplateModal(id = null, listId = null) {
    var select = document.getElementById('email_tpl_list_select');
    select.innerHTML = '<option value="">Sem Categoria</option>';
    allEmailLists.forEach(l => {
        select.innerHTML += `<option value="${l.id}">${l.name}</option>`;
    });
    
    if(id) {
        var tpl = allEmailTemplates.find(t => t.id == id);
        if(tpl) {
            document.getElementById('email_tpl_id').value = tpl.id;
            document.getElementById('email_tpl_title').value = tpl.titulo;
            document.getElementById('email_tpl_body').value = tpl.corpo;
            document.getElementById('email_tpl_list_select').value = tpl.list_id || '';
        }
    } else {
        document.getElementById('email_tpl_id').value = '';
        document.getElementById('email_tpl_title').value = '';
        document.getElementById('email_tpl_body').value = '';
        document.getElementById('email_tpl_list_select').value = listId || '';
    }
    
    modalEmailTemplate.show();
}

function saveEmailTemplate() {
    var id = document.getElementById('email_tpl_id').value;
    var title = document.getElementById('email_tpl_title').value;
    var body = document.getElementById('email_tpl_body').value;
    var list_id = document.getElementById('email_tpl_list_select').value;
    
    var fd = new FormData();
    fd.append('acao', 'ajax_save_email_template');
    fd.append('id', id);
    fd.append('titulo', title);
    fd.append('corpo', body);
    fd.append('list_id', list_id);
...
function deleteEmailTemplate(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            var fd = new FormData();
            fd.append('acao', 'ajax_delete_email_template');
            fd.append('id', id);
            fetch('', { method: 'POST', body: fd })
            .then(r => r.json())
            .then(res => {
...
function selectEmailTemplateNew(id) {
    if(isEmailEditMode) return;
    
    if(!document.getElementById('proc_port')) {
        Swal.fire('Aviso', 'Abra um processo primeiro.', 'warning');
        return;
    }
    var port = document.getElementById('proc_port').value;
    if(!port) {
    ...
    showLoading();
    let fd = new FormData();
    fd.append('acao', 'ajax_generate_email_text');
    fd.append('tpl_id', id);
    ...
    .then(res => {
        hideLoading();
        if(res.status == 'ok') {
            document.getElementById('email_body').value = res.text;
            document.getElementById('email_subject').value = res.subject;
        } else {
            Swal.fire('Erro', res.text, 'error');
        }
    }).catch(err => {
        hideLoading();
        console.error(err);
...
<!-- Modal Manage Greetings -->
<div class="modal fade" id="modalManageGreetings" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-navy text-white">
                <h5 class="modal-title">Gerenciar Saudações e Fechamentos</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <ul class="nav nav-tabs mb-3" id="greetingsTab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active" id="tab-greeting-list" data-bs-toggle="tab" data-bs-target="#greeting-list-pane" type="button" role="tab">Saudações</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="tab-closing-list" data-bs-toggle="tab" data-bs-target="#closing-list-pane" type="button" role="tab">Fechamentos</button>
                    </li>
                </ul>
                <div class="tab-content" id="greetingsTabContent">
                    <div class="tab-pane fade show active" id="greeting-list-pane" role="tabpanel">
                        <div class="d-flex justify-content-end mb-2">
                            <button class="btn btn-sm btn-primary" onclick="openGreetingEdit(null, 'greeting')"><i class="fas fa-plus"></i> Nova Saudação</button>
                        </div>
                        <div id="greetings_list_container" class="list-group"></div>
                    </div>
                    <div class="tab-pane fade" id="closing-list-pane" role="tabpanel">
                        <div class="d-flex justify-content-end mb-2">
                            <button class="btn btn-sm btn-primary" onclick="openGreetingEdit(null, 'closing')"><i class="fas fa-plus"></i> Novo Fechamento</button>
                        </div>
                        <div id="closings_list_container" class="list-group"></div>
                    </div>
                </div>
                
                <div id="greeting_edit_area" style="display:none;" class="mt-4 pt-4 border-top">
                    <h6 class="text-navy fw-bold mb-3" id="greeting_edit_title">Editar</h6>
                    <input type="hidden" id="gr_id">
                    <input type="hidden" id="gr_type">
                    <div class="mb-3">
                        <label class="form-label">Título</label>
                        <input type="text" class="form-control" id="gr_title">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Conteúdo (Use {Nome}, {ID}, etc)</label>
                        <textarea class="form-control" id="gr_body" rows="3"></textarea>
                    </div>
                    <div class="text-end">
                        <button class="btn btn-secondary me-2" onclick="cancelGreetingEdit()">Cancelar</button>
                        <button class="btn btn-success" onclick="saveGreeting()">Salvar</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
var modalManageGreetings = new bootstrap.Modal(document.getElementById('modalManageGreetings'));
var allGreetings = [];
var currentDefaults = {greeting: null, closing: null};

function openGreetingsManager() {
    loadGreetingsData();
    modalManageGreetings.show();
}
...
function loadGreetingsData() {
    fetch('', {
        method: 'POST', 
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: 'acao=ajax_get_greetings'
    })
    .then(r => r.json())
    .then(res => {
        if(res.status == 'ok') {
            allGreetings = res.data;
            currentDefaults = res.defaults;
            renderGreetingsLists();
        }
    });
}

function renderGreetingsLists() {
    renderGreetingsList(allGreetings.filter(g => g.type == 'greeting'), 'greeting', 'greetings_list_container', currentDefaults.greeting);
    renderGreetingsList(allGreetings.filter(g => g.type == 'closing'), 'closing', 'closings_list_container', currentDefaults.closing);
}

function renderGreetingsList(items, type, containerId, defaultId) {
    var container = document.getElementById(containerId);
    container.innerHTML = '';
    
    if(items.length === 0) {
        container.innerHTML = '<div class="text-muted text-center p-3">Nenhum registro.</div>';
        return;
    }
    
    items.forEach(item => {
        var isDef = (item.id == defaultId);
        var defBadge = isDef ? '<span class="badge bg-success ms-2">Padrão</span>' : '';
        var defBtn = isDef ? '' : `<button class="btn btn-sm btn-outline-success me-2" onclick="setDefaultGreeting('${item.id}', '${type}')" title="Definir como Padrão"><i class="fas fa-check"></i></button>`;
        ...
        var html = `
            <div class="list-group-item d-flex justify-content-between align-items-center">
                <div>
                    ...
                    ${defBadge}
                    <div class="small text-muted text-truncate" style="max-width: 400px;">${item.corpo}</div>
                </div>
                <div class="text-nowrap">
                    ${defBtn}
                    <button class="btn btn-sm btn-outline-primary me-1" onclick="openGreetingEdit(${safeItem}, '${type}')"><i class="fas fa-pen"></i></button>
                    <button class="btn btn-sm btn-outline-danger" onclick="deleteGreeting('${item.id}')"><i class="fas fa-trash"></i></button>
                </div>
            </div>
        `;
...
function openGreetingEdit(item, type) {
    document.getElementById('greeting_edit_area').style.display = 'block';
    
    if(item) {
        document.getElementById('greeting_edit_title').innerText = 'Editar ' + (type=='greeting' ? 'Saudação' : 'Fechamento');
        document.getElementById('gr_id').value = item.id;
        document.getElementById('gr_title').value = item.titulo;
        document.getElementById('gr_body').value = item.corpo;
        document.getElementById('gr_type').value = item.type;
    } else {
        document.getElementById('greeting_edit_title').innerText = 'Nova ' + (type=='greeting' ? 'Saudação' : 'Fechamento');
        document.getElementById('gr_id').value = '';
        document.getElementById('gr_title').value = '';
        document.getElementById('gr_body').value = '';
        document.getElementById('gr_type').value = type;
    }
    document.getElementById('gr_title').focus();
}

function cancelGreetingEdit() {
    document.getElementById('greeting_edit_area').style.display = 'none';
}

function saveGreeting() {
    var id = document.getElementById('gr_id').value;
    var title = document.getElementById('gr_title').value;
    var body = document.getElementById('gr_body').value;
    var type = document.getElementById('gr_type').value;
    
    if(!title) { Swal.fire('Erro', 'Título obrigatório', 'error'); return; }
    
    var fd = new FormData();
    fd.append('acao', 'ajax_save_greeting');
    fd.append('id', id);
    fd.append('titulo', title);
    fd.append('corpo', body);
    fd.append('type', type);
...
function deleteGreeting(id) {
    Swal.fire({
        ...
        if(r.isConfirmed) {
            var fd = new FormData();
            fd.append('acao', 'ajax_delete_greeting');
            fd.append('id', id);
            fetch('', { method: 'POST', body: fd })
            .then(r => r.json())
            .then(res => {
                if(res.status == 'ok') loadGreetingsData();
            });
        }
    });
}

function setDefaultGreeting(id, type) {
    var fd = new FormData();
    fd.append('acao', 'ajax_set_default_greeting');
    fd.append('id', id);
    fd.append('type', type);
    fetch('', { method: 'POST', body: fd })
    .then(r => r.json())
    .then(res => {
        if(res.status == 'ok') {
            loadGreetingsData();
            Swal.fire({title: 'Padrão atualizado', icon: 'success', toast: true, position: 'top-end', showConfirmButton: false, timer: 1500});
        }
    });
}
</script>

<!-- Modal Greeting Selector -->
<div class="modal fade" id="modalGreetingSelector" tabindex="-1">
    <div class="modal-dialog modal-sm modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-navy text-white">
                <h5 class="modal-title fs-6">Configurar Saudação/Fechamento</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <div class="mb-3">
                    <label class="form-label small">Saudação</label>
                    <select id="sel_greeting" class="form-select form-select-sm" onchange="applyGreetingSelection()">
                        <option value="">(Nenhuma)</option>
                    </select>
                </div>
                <div class="mb-3">
                    <label class="form-label small">Fechamento</label>
                    <select id="sel_closing" class="form-select form-select-sm" onchange="applyGreetingSelection()">
...
<script>
var selectedGreetingId = null;
var selectedClosingId = null;
var modalGreetingSelector = new bootstrap.Modal(document.getElementById('modalGreetingSelector'));

function openGreetingSelector() {
    loadGreetingsDataForSelector();
}

function loadGreetingsDataForSelector() {
    fetch('', {
        method: 'POST', 
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: 'acao=ajax_get_greetings'
    ...
    .then(res => {
        if(res.status == 'ok') {
            allGreetings = res.data;
            if (selectedGreetingId === null) selectedGreetingId = res.defaults.greeting;
            if (selectedClosingId === null) selectedClosingId = res.defaults.closing;
            
            populateSelector('sel_greeting', allGreetings.filter(g => g.type == 'greeting'), selectedGreetingId);
            populateSelector('sel_closing', allGreetings.filter(g => g.type == 'closing'), selectedClosingId);
            
            modalGreetingSelector.show();
        }
    });
}

function populateSelector(id, items, selectedId) {
...
function applyGreetingSelection() {
    selectedGreetingId = document.getElementById('sel_greeting').value || null;
    selectedClosingId = document.getElementById('sel_closing').value || null;
    generateText();
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    .then(res => {
        if(res.status == 'ok') {
            if (res.defaults.greeting) selectedGreetingId = res.defaults.greeting;
            if (res.defaults.closing) selectedClosingId = res.defaults.closing;
        }
    });
});
</script>
