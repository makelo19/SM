...
if (isset($_POST['acao']) && strpos($_POST['acao'], 'ajax_') === 0) {
    ...
    if ($act == 'ajax_save_process_data_record') {
        ...
        if ($isUpdate) {
             $rows = $db->readJSON($db->getPath($targetFile));
             $found = false;
             foreach ($rows as &$r) {
                 $rId = isset($r['UID']) ? $r['UID'] : md5(json_encode($r));
                 if ($rId === $uid) {
                     if (!isset($r['UID'])) $r['UID'] = $uid; // Ensure UID persistence
                     $r = array_merge($r, $record);
                     $found = true;
                     break;
                 }
             }
             if ($found && $db->writeJSON($db->getPath($targetFile), $rows)) {
                 $success = true;
             }
        } else {
        ...
        if ($success) {
            ...
            if ($file) {
                 $db->update($file, $ID_FIELD_KEY, $port, ['Ultima_Alteracao' => date('d/m/Y H:i:s'), 'Nome_atendente' => $_SESSION['nome_completo']]);
            }
