...
if (isset($_POST['acao']) && strpos($_POST['acao'], 'ajax_') === 0) {
 ...
 usort($processos, function($a, $b) use ($sortCol, $desc, $clientMap) {
        ...
        if(!isset($schemaCols['Nome_atendente'])) $schemaCols['Nome_atendente'] = ['key'=>'Nome_atendente', 'label'=>'Atendente'];
        ...
        foreach($processos as $proc) {
            ...
            foreach($dashColumns as $col) {
                ...
                if($colKey === $ID_FIELD_KEY) {
                    ...
                    echo htmlspecialchars(format_field_value($ID_FIELD_KEY, $port));
                    if($cred) {
                         $cIcon = $IDENT_ICON; 
                    ...
                    echo '<td class="dashboard-text">' . htmlspecialchars(function_exists('format_field_value') ? format_field_value('DATA', get_value_ci($proc, 'DATA')) : get_value_ci($proc, 'DATA')) . '</td>';
                    ...
                    if ($colKey === 'STATUS') {
                        echo '<td><span class="badge bg-secondary status-badge">' . htmlspecialchars(format_field_value($colKey, $val)) . '</span></td>';
                    } else {
                ...
                <div class="card card-custom p-3 h-100">
                    ...
                    <ul class="list-group list-group-flush mb-3 sortable-list" data-file="<?= $file ?>">
                        <?php foreach($config->getFields($file) as $f): 
                            ...
                            // Hide Fixed Internal Fields from Configuration UI
                            if ($file === 'Base_processos_schema' && in_array($f['key'], ['Data_Lembrete', 'Ultima_Alteracao'])) continue;
                    ...
                    <div class="tab-pane fade show active" id="tab-dados">
                        ...
                        <form method="POST" id="form_processo">
                            ...
                            if (!$firstIsTitle) {
                                 ...
                                 echo '<h5 class="text-navy fw-bold border-bottom pb-2">Dados do Processo' . $lembreteIcon . '</h5>';
