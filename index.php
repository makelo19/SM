...
if (isset($_GET['acao']) && $_GET['acao'] == 'exportar_excel') {
    ...
    // Headers
    $emailHeaders = ['Data e Hora', $IDENT_LABEL, 'Responsável', 'Assunto', 'Para', 'CC'];
    echo "  <Row>\n";
...
function showEmailContactForm(c) {
    ...
    if(c) {
        ...
        document.getElementById('ecm_id').value = c._id || c.UID || '';
...
function deleteEmailContactManager(id) {
    Swal.fire({
        ...
        if(r.isConfirmed) {
            ...
            Swal.showLoading();
            var fd = new FormData();
            fd.append('acao', 'ajax_delete_base_records_json');
            fd.append('base', 'Base_contatos_email.json');
            fd.append('ids[]', id);
            fetch('', {method: 'POST', body: fd})
            .then(r => r.json())
