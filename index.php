...
// SM Settings - Configurable system parameters (load early so other classes can use them)
...
if (isset($_POST['acao']) && strpos($_POST['acao'], 'ajax_') === 0) {
    ...
    if ($act == 'ajax_check_process') {
        ...
        if (!$data) {
             ...
             $creditData = $db->find('Identificacao_cred.json', $IDENT_ID_FIELD, $port);
        }
    ...
    if ($act == 'ajax_delete_credit_bulk') {
        ...
        ob_clean(); header('Content-Type: application/json');
        if (!empty($ports)) {
            if ($db->deleteMany('Identificacao_cred.json', $IDENT_ID_FIELD, $ports)) {
                // Cleanup Uploads
                foreach($ports as $p) {
                    $uDir = 'upload/' . $p;
    ...
    if ($act == 'ajax_save_history') {
        ...
        $id = $templates->recordHistory($_SESSION['nome_completo'], $_POST['cliente'], $_POST['cpf'], $_POST['port'], $_POST['modelo'], $_POST['texto'], $_POST['destinatarios'] ?? '', $extra);
    ...
    if ($act == 'ajax_save_credit') {
        ...
        if ($originalPort) {
             ...
             if ($port != $originalPort) {
                 $exists = $db->find('Identificacao_cred.json', $IDENT_ID_FIELD, $port);
                 if ($exists) {
                     echo json_encode(['status'=>'error', 'message'=>'Nova ' . htmlspecialchars($IDENT_LABEL) . ' já existe na base.']);
                     exit;
                 }
             }
             $res = $db->update('Identificacao_cred.json', $IDENT_ID_FIELD, $originalPort, $data);
    ...
    if ($act == 'ajax_delete_credit') {
        ...
        if (!empty($ports)) { // Check if $ports is not empty
            if ($db->deleteMany('Identificacao_cred.json', $IDENT_ID_FIELD, $ports)) { // Changed from delete to deleteMany
 ...
 usort($processos, function($a, $b) use ($sortCol, $desc, $clientMap) {
        ...
        $clientes = $db->findMany('Base_clientes.json', 'CPF', $cpfs);
        $clientMap = []; foreach ($clientes as $c) $clientMap[$c['CPF']] = $c['Nome'];
        $creditos = $db->findMany('Identificacao_cred.json', $IDENT_ID_FIELD, $ports);
        $creditoMap = []; foreach ($creditos as $c) $creditoMap[$c[$IDENT_ID_FIELD]] = $c;

        $dashColumns = $settings->getDashboardColumns();
    ...
    if ($act == 'ajax_get_process_full') {
        ...
        $credit = $db->find('Identificacao_cred.json', $IDENT_ID_FIELD, $port);
    ...
    if ($act == 'ajax_render_base_table') {
        ...
        $bFilters = $settings->getBaseFilters();
        ...
        if (!empty($_POST['fAtendente'])) $dynamicRules['Nome_atendente'] = $_POST['fAtendente'];
        if (!empty($_POST['fStatus'])) $dynamicRules['STATUS'] = $_POST['fStatus']; // Support legacy/fixed status input if distinct from bFilters
        
        // Composite Callback for Dynamic Rules
        $checkDynamicBase = function($row) use ($dynamicRules) {
        ...
        if ($fDataIni || $fDataFim) {
             ...
             if ($dateCol) {
                  ...
                  $filters['callback'] = function($row) use ($di, $df, $dateCol, $checkDynamicBase) {
                     ...
                     if (!$d) return false;
                     $dt = DateTime::createFromFormat('!d/m/Y', $d);
                     if (!$dt) $dt = DateTime::createFromFormat('d/m/Y H:i', $d); // Input fallback
                     if (!$dt) return false;
        ...
        if ($base === 'Processos') {
             // Configurable Columns + Fixed Constraints (ID, Date, Atendente, Action)
             $dashCols = $settings->getDashboardColumns(); // Reuse dashboard config for Registros view as requested
             
             // Build Config Fields ensuring Fixed Order: ID, Date, Atendente, [Config], Action(handled by UI)
             // ID is handled by logic as PK, usually rendered but we can explicit it.
             // Line 2004 renders PK checkbox.
             // The loop renders OTHER columns.
             
             $confFields = [];
             
             // Fixed: ID
             $confFields[] = ['key' => $ID_FIELD_KEY, 'label' => $ID_LABEL];
             // Fixed: Date/Time
             $confFields[] = ['key' => 'DATA', 'label' => 'Data/Hora'];
             // Fixed: Atendente
             $confFields[] = ['key' => 'Nome_atendente', 'label' => 'Atendente'];
             
             // Configurable
             foreach($dashCols as $dc) {
                 if (in_array($dc['key'], [$ID_FIELD_KEY, 'DATA', 'Nome_atendente'])) continue;
                 $confFields[] = $dc;
             }
        } else {
            // For Visualizacao (Identificacao) etc, keep Schema based or use base_columns if preferred?
            // User didn't strictly specify columns for Visualizacao, only Filters.
            // But we can support base_columns setting.
            // Let's stick to existing Schema logic for safety/compatibility unless base_columns is set?
            // Existing logic loops all fields.
        ...
        foreach($rows as $r) {
            ...
            foreach($confFields as $f) {
                 ...
                 if ($base === 'Processos') {
                     if ($f['key'] === 'STATUS') {
                         echo '<td><span class="badge bg-secondary">' . htmlspecialchars($val) . '</span></td>';
                         continue;
                     }
    ...
    if ($act == 'ajax_salvar_campo') {
        ...
        $fieldData = [
            ...
            'show_column' => isset($_POST['show_column']),
            'show_filter' => isset($_POST['show_filter'])
        ];
        ...
        // SYNC SETTINGS (Auto-update Dashboard Configs)
        if ($file === 'Base_processos_schema') {
             $bFields = $config->getFields($file);
             
             // 1. Dashboard Columns (Fixed + Configured)
             $cols = [
                 ['key' => $ID_FIELD_KEY, 'label' => $settings->getIdLabel()],
                 ['key' => 'DATA', 'label' => 'Data/Hora'],
                 ['key' => 'Nome_atendente', 'label' => 'Atendente']
             ];
             foreach($bFields as $bf) {
                 if (!empty($bf['show_column']) && !in_array($bf['key'], [$ID_FIELD_KEY, 'DATA', 'Nome_atendente'])) {
                     $cols[] = ['key'=>$bf['key'], 'label'=>$bf['label']];
                 }
             }
             $settings->set('dashboard_columns', $cols);
             
             // 2. Visible Filters (Fixed are hardcoded, we sync Dynamic)
             $filts = []; 
             foreach($bFields as $bf) {
                 if (!empty($bf['show_filter']) && !in_array($bf['key'], [$ID_FIELD_KEY, 'DATA', 'Nome_atendente'])) {
                     $filts[] = $bf['key'];
                 }
             }
             $settings->set('visible_process_filters', $filts);
        } else {
             // For Base Schemas (Identificacao, etc), sync to base_filters if desired
             // Heuristic: If Base schema has show_filter, add to base_filters
             $bFields = $config->getFields($file);
             $filts = [];
             foreach($bFields as $bf) {
                 if (!empty($bf['show_filter'])) $filts[] = $bf['key'];
             }
             if (!empty($filts)) {
                 // For now, base_filters is global. We might overwrite specific base filters?
                 // Current system has single 'base_filters' setting. 
                 // We should probably rely on User manually setting it OR just use the Process logic universally?
                 // Let's NOT touch base_filters for non-process files to avoid breaking existing logic unknowingly.
                 // Only if user explicitly requests Base Config Sync.
                 // Given Request: "Bases > Gestão de Bases > Visualização... Campos filtro configuraveis".
                 // Identificacao_cred.json usually has NO schema file managed by Config class unless created manually.
                 // If managed, we sync.
             }
        }
        exit;
    ...
    if ($act == 'ajax_render_config') {
        ...
        <div class="row mb-5">
            <?php foreach(['Base_processos_schema' => 'Processos', 'Base_registros_schema' => 'Campos de Registros', 'Identificacao_cred.json' => $IDENT_LABEL] as $file => $label): ?>
            <div class="col-md-3">
                <div class="card card-custom p-3 h-100">
                    ...
                    <ul class="list-group list-group-flush mb-3 sortable-list" data-file="<?= $file ?>">
                        <?php foreach($config->getFields($file) as $f): 
                            if(isset($f['deleted']) && $f['deleted']) continue;
                            $lockedFields = [$ID_FIELD_KEY]; // Only the ID field is protected 
...
if (isset($_GET['acao']) && $_GET['acao'] == 'download_credito_template') {
    ...
    echo "Status;Número Depósito;Data Depósito;Valor Depósito Principal;Texto Pagamento;" . $IDENT_LABEL . ";Certificado;Status 2\n";
    exit;
}
        ...
        if ($text) {
            ...
            foreach ($rows as $cols) {
                ...
                if (stripos($base, 'client') !== false) {
                    ...
                    if (empty($newRow[$ID_FIELD_KEY])) continue;
                } else {
                    if (empty($newRow[$IDENT_ID_FIELD])) continue;
                }
...
<div class="container-fluid px-4">
    ...
    <div id="page-dashboard" class="page-section" style="<?= $page=='dashboard'?'':'display:none' ?>">
        ...
        $visFilters = $settings->getVisibleProcessFilters();
        
        $dynamicCheck = function($row) use ($visFilters) {
            foreach($visFilters as $vk) {
                // Skip if fixed (Status/Atendente/Data handled elsewhere, but Status handled via $filters array above)
        ...
        $clientes = $db->findMany('Base_clientes.json', 'CPF', $cpfs);
        $clientMap = []; foreach ($clientes as $c) $clientMap[$c['CPF']] = $c['Nome'];
        $dashCreditos = $db->findMany('Identificacao_cred.json', $IDENT_ID_FIELD, $ports);
            ...
            <form onsubmit="filterDashboard(event)" class="row g-3" id="form_dashboard_filter">
                ...
                foreach ($visFilters as $vk) {
                    ...
                    if (isset($fieldMap[$vk])) {
                         ...
                         if (!empty($fieldMap[$vk]['options'])) $fOptions = explode(',', $fieldMap[$vk]['options']);
                    }
                    ...
                    if (is_array($fOptions)) {
                         ...
                         echo '<input type="text" name="f_' . htmlspecialchars($vk) . '" class="form-control form-control-sm" value="' . htmlspecialchars($val) . '">';
                    }
                    echo '</div>';
                }
                ?>
        ...
        <div class="card card-custom p-4">
            ...
            <div class="table-responsive">
                <table class="table table-hover table-custom align-middle" id="dash_table_head">
                    <thead><tr>
                        <th class="sortable-header" data-col="<?= htmlspecialchars($ID_FIELD_KEY) ?>" onclick="setDashSort('<?= htmlspecialchars($ID_FIELD_KEY) ?>')" style="cursor:pointer"><?= htmlspecialchars($ID_LABEL) ?> <span class="sort-icon"><i class="fas fa-sort-down text-dark ms-1"></i></span></th>
                        <th class="sortable-header" data-col="DATA" onclick="setDashSort('DATA')" style="cursor:pointer">Data/Hora <span class="sort-icon"><i class="fas fa-sort text-muted ms-1" style="opacity:0.3"></i></span></th>
                        ...
                        <?php 
                        $dashCols = $settings->getDashboardColumns();
                        foreach($dashCols as $col): 
                            if(in_array($col['key'], [$ID_FIELD_KEY, 'DATA', 'Nome_atendente'])) continue; 
                        ?>
                        <th class="sortable-header" data-col="<?= htmlspecialchars($col['key']) ?>" onclick="setDashSort('<?= htmlspecialchars($col['key']) ?>')" style="cursor:pointer"><?= htmlspecialchars($col['label']) ?> <span class="sort-icon"><i class="fas fa-sort text-muted ms-1" style="opacity:0.3"></i></span></th>
                        <?php endforeach; ?>
                        <th>Ação</th>
                    </tr></thead>
                    <tbody id="dash_table_body">
                        ...
                        ?>
                        <tr class="<?= $rowClass ?>">
                            <td class="dashboard-text fw-bold"><?= htmlspecialchars($procIdVal) ?> <?php if($cred): ?><i class="<?= htmlspecialchars($IDENT_ICON) ?> ms-2" style="color:var(--laranja); animation: blink 1.5s infinite;" title="<?= htmlspecialchars($IDENT_LABEL) ?> Encontrada!"></i><?php endif; ?></td>
                            <td class="dashboard-text"><?= htmlspecialchars($proc['DATA'] ?? '') ?></td>
                            ...
                            <?php foreach($dashCols as $col): 
                                if(in_array($col['key'], [$ID_FIELD_KEY, 'DATA', 'Nome_atendente'])) continue;
                                $colVal = get_value_ci($proc, $col['key']);
                                if ($col['key'] === 'STATUS') {
                                    echo '<td><span class="badge bg-secondary status-badge">' . htmlspecialchars($colVal) . '</span></td>';
                                } else {
                                    echo '<td class="dashboard-text">' . htmlspecialchars($colVal) . '</td>';
                                }
                            endforeach; ?>
                        ...
                        <?php endforeach; ?>
                        <?php if(empty($dashProcessos)): ?><tr><td colspan="<?= count($dashCols) + 4 // + ID, Data, Atent, Action ?>" class="text-center text-muted py-4">Nenhum registro encontrado.</td></tr><?php endif; ?>
                    </tbody>
                </table>
            </div>
    ...
    <div id="page-detalhes" class="page-section" style="<?= $page=='detalhes'?'':'display:none' ?>">
        ...
        $credito = $id ? $db->find('Identificacao_cred.json', $IDENT_ID_FIELD, $id) : null;
        ...
        <div class="row">
            <div class="col-12 mb-3 d-flex align-items-center gap-2 flex-wrap">
                ...
                <div class="ms-auto d-flex align-items-center gap-2" style="min-width: 350px;">
                    <div class="input-group">
                        <span class="input-group-text bg-white"><i class="fas fa-search text-muted"></i></span>
                        <input type="text" class="form-control" id="global_search_servicos" placeholder="<?= htmlspecialchars($settings->get('id_search_label', 'Buscar por ' . $ID_LABEL)) ?>" value="<?= htmlspecialchars($id) ?>">
                        <button class="btn btn-navy" onclick="globalSearchServicos()"><i class="fas fa-arrow-right"></i></button>
                    </div>
                </div>
            </div>
            <div class="col-md-12">
                    ...
                    <div class="tab-pane fade show active" id="tab-dados">
                        ...
                        <form method="POST" id="form_processo">
                            ...
                            if (!$firstIsTitle) {
                                 ...
                                 echo '<h5 class="text-navy fw-bold border-bottom pb-2">Dados do Processo</h5>';
                                ...
                                $val = $getVal($processo, $f['key']);
                                    ...
                                    <div class="col-md-4 <?= $hideClass ?>" data-field-key="<?= $f['key'] ?>">
                                        ...
                                        <?php if($f['key'] == $ID_FIELD_KEY): ?>
                                            <div class="input-group">
                                                <input type="text" name="<?= htmlspecialchars($ID_FIELD_KEY) ?>" id="proc_port" class="form-control form-control-custom fw-bold" value="<?= htmlspecialchars($val ?: ($id ?? '')) ?>" required placeholder="<?= htmlspecialchars($settings->get('id_search_label', 'Buscar/Criar...')) ?>">
                                        ...
                                        <?php else: ?>
                                            <input type="text" name="<?= $f['key'] ?>" id="proc_<?= $f['key'] ?>" class="form-control form-control-custom" value="<?= htmlspecialchars($val) ?>" <?= $req ?> <?= $disabled ?>>
                                        <?php endif; ?>
                                    </div>
                                    <?php endforeach; ?>
                                    <?php 
                            ...
                            <div class="d-flex justify-content-between">
                                <div id="div_delete_process">
                                    <?php if($processo): ?>
                                    <button type="button" class="btn btn-outline-danger" onclick="confirmDelete('<?= htmlspecialchars(get_value_ci($processo, $ID_FIELD_KEY)) ?>')">Excluir Processo</button>
                                    <?php endif; ?>
                                </div>
                    ...
                    <div class="tab-pane fade" id="tab-registros">
                        <div class="card card-custom p-4 mb-4">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                ...
                                <button class="btn btn-sm btn-outline-danger" onclick="deleteSelectedHistory('registros')"><i class="fas fa-trash me-1"></i> Excluir Selecionados</button>
                            </div>
                            <div class="table-responsive">
                    ...
                    <div class="tab-pane fade" id="tab-textos">
                        <div class="card card-custom p-4 mb-4">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                ...
                                <button class="btn btn-sm btn-outline-danger" onclick="deleteSelectedHistory('envios')"><i class="fas fa-trash me-1"></i> Excluir Selecionados</button>
                            </div>
                            <div class="table-responsive">
                                <table class="table table-sm table-striped small">
                                    <thead><tr>
    ...
    <div id="page-base" class="page-section" style="<?= $page=='base'?'':'display:none' ?>">
       ...
       <div class="card card-custom p-3 mb-4">
           <ul class="nav nav-pills nav-fill gap-3" id="base-tab">
               <li class="nav-item">
                   <button class="nav-link active rounded-pill" id="tab-cred" onclick="switchBase('Identificacao_cred.json')"><?= htmlspecialchars($IDENT_LABEL) ?></button>
               </li>
               <li class="nav-item">
       ...
       <div class="card card-custom p-4 mb-4">
            ...
            <form onsubmit="filterBase(event)" class="row g-3 align-items-end" id="form_base_filter">
                ...
                <?php 
                $bFilters = $settings->getBaseFilters();
                // We render them universally. User configures them for Visualizacao.
                foreach($bFilters as $bk) {
                     $label = $bk;
                     $opts = null;
                     // Try to find label/options from current base schema? 
                     // But we don't know current base here (defaults to Identificacao).
                     // We can't easily guess options efficiently without loading schema.
                     // But for 'STATUS', we can use #base_fStatus ID for JS population.
                     
                     $idAttr = '';
                     if ($bk === 'STATUS') $idAttr = 'id="base_fStatus"';
                     
                     echo '<div class="col-md-3">';
                     echo '<label>' . htmlspecialchars($label) . '</label>';
                     
                     if ($bk === 'STATUS') {
                         echo '<select name="f_' . htmlspecialchars($bk) . '" ' . $idAttr . ' class="form-select"></select>';
                     } else {
                         echo '<input type="text" name="f_' . htmlspecialchars($bk) . '" class="form-control">';
                     }
                     echo '</div>';
                }
                ?>
    ...
    <div class="modal-dialog">
        <form class="modal-content" method="POST" onsubmit="event.preventDefault(); submitConfigForm(this, 'ajax_salvar_campo')">
            ...
            <div class="modal-body">
                ...
                <div class="form-check mb-2" id="div_field_show_reminder">
                ...
                <!-- Options below Lembretes -->
                <div class="form-check mb-2" id="div_field_show_column">
                    <input class="form-check-input" type="checkbox" name="show_column" id="field_show_column">
                    <label class="form-check-label" for="field_show_column">Exibir em Colunas (Grade)</label>
                </div>
                <div class="form-check mb-3" id="div_field_show_filter">
                    <input class="form-check-input" type="checkbox" name="show_filter" id="field_show_filter">
                    <label class="form-check-label" for="field_show_filter">Exibir em Filtros</label>
                </div>
            </div>
            <div class="modal-footer"><button class="btn btn-navy">Salvar</button></div>
        </form>
...
<script>
    ...
    function addFieldModal(file) { 
        ...
        document.getElementById('field_show_column').checked = false;
        document.getElementById('field_show_filter').checked = false;
    ...
    function editField(f, file) { 
        ...
        document.getElementById('field_show_column').checked = (f.show_column === true || f.show_column === "true");
        document.getElementById('field_show_filter').checked = (f.show_filter === true || f.show_filter === "true");
        new bootstrap.Modal(document.getElementById('modalField')).show(); 
    }
            ...
            if (result.isConfirmed) {
                ...
                .then(res => {
                    if(res.status == 'ok') {
                         ...
                         // Refresh to re-index
                         var p = document.getElementById('proc_port');
                         if(p && p.value) loadProcess(p.value);
                    } else {
