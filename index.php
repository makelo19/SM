...
$attachmentManager = new AttachmentManager('dados/anexos');
...
if (isset($_POST['acao']) && strpos($_POST['acao'], 'ajax_') === 0) {
    ...
    if ($act == 'ajax_save_system_config') {
        $systemConfig->setAppName($_POST['app_name']);
        $systemConfig->setPKField($_POST['pk_field']);
        $systemConfig->setCurrencySymbol($_POST['currency_symbol']);
        ob_clean(); header('Content-Type: application/json');
        echo json_encode(['status' => 'ok', 'message' => 'Configurações do sistema atualizadas!']);
        exit;
    }
    ...
    if ($act == 'ajax_check_cpf_processes') {
        ...
        foreach($rows as $r) {
            $result[] = [
                'port' => get_value_ci($r, $systemConfig->getPKField()),
    ...
    if ($act == 'ajax_check_process') {
        ...
        $sysPK = $systemConfig->getPKField();
        
        $data = null;
        $file = $indexer->get($port);
        
        if ($file) {
            $res = $db->select($file, [$sysPK => $port], 1, 1);
        ...
        if (!$data) {
            $files = $db->getAllProcessFiles();
            $foundFile = $db->findFileForRecord($files, $sysPK, $port);
            
            if ($foundFile) {
                $res = $db->select($foundFile, [$sysPK => $port], 1, 1);
                if (!empty($res['data'])) {
                    $data = $res['data'][0];
    ...
    if ($act == 'ajax_check_cert') {
        ...
        $results = [];
        foreach($rows as $r) {
            $results[] = ['port' => get_value_ci($r, $systemConfig->getPKField()), 'data' => get_value_ci($r, 'DATA'), 'status' => get_value_ci($r, 'STATUS'), 'source' => 'Processo'];
        }
    ...
    if ($act == 'ajax_save_history') {
        ...
        if (!$file) {
             ...
             $months = $_SESSION['selected_months'] ?? [(int)date('n')];
             $files = $db->getProcessFiles($years, $months);
             $file = $db->findFileForRecord($files, $sysPK, $port);
        }
        if ($file) {
             $db->update($file, $sysPK, $port, ['Ultima_Alteracao' => date('d/m/Y H:i:s'), 'Nome_atendente' => $_SESSION['nome_completo']]);
        }
    ...
    // ANEXOS HANDLERS
    if ($act == 'ajax_list_anexos') {
        $port = $_POST['port'] ?? '';
        $files = $attachmentManager->listFiles($port);
        ob_clean(); header('Content-Type: application/json');
        echo json_encode(['status'=>'ok', 'files'=>$files, 'download_url' => '?acao=download_anexo&port='.urlencode($port).'&name=']);
        exit;
    }

    if ($act == 'ajax_upload_anexo') {
        $port = $_POST['port'] ?? '';
        if (!$port || empty($_FILES['file'])) {
            ob_clean(); header('Content-Type: application/json');
            echo json_encode(['status'=>'error', 'message'=>'Dados inválidos.']);
            exit;
        }
        
        if ($attachmentManager->upload($port, $_FILES['file'])) {
            ob_clean(); header('Content-Type: application/json');
            echo json_encode(['status'=>'ok', 'message'=>'Upload concluído!']);
        } else {
            ob_clean(); header('Content-Type: application/json');
    ...
    if ($act == 'ajax_delete_anexo') {
        $port = $_POST['port'];
        $name = $_POST['name'];
        ob_clean(); header('Content-Type: application/json');
        if ($attachmentManager->delete($port, $name)) {
            echo json_encode(['status'=>'ok']);
        } else {
            echo json_encode(['status'=>'error', 'message'=>'Arquivo não encontrado ou erro.']);
        }
        exit;
    }

    if ($act == 'ajax_rename_anexo') {
        $port = $_POST['port'];
        $old = $_POST['old'];
        $new = $_POST['new'];
        ob_clean(); header('Content-Type: application/json');
        if ($attachmentManager->rename($port, $old, $new)) {
            echo json_encode(['status'=>'ok']);
        } else {
    ...
    if ($act == 'ajax_salvar_processo') { 
        ...
        $cpf = $_POST['CPF'] ?? '';
        $sysPK = $systemConfig->getPKField();
        // Try exact key first, then underscored
        $port = $_POST[$sysPK] ?? ($_POST[str_replace(' ', '_', $sysPK)] ?? '');
        $ag = $_POST['AG'] ?? '';

        if (!$port) {
            echo json_encode(['status'=>'error', 'message'=>"Erro: $sysPK é obrigatório."]);
            exit;
        } 
        ...
        if ($foundFile) {
            ...
            if ($foundFile !== $targetFile) {
                ...
                $oldData = $db->find($foundFile, $sysPK, $port);
                $fullData = $oldData ? array_merge($oldData, $data) : $data;

                $db->delete($foundFile, $sysPK, $port);
                ...
                $db->update($foundFile, $sysPK, $port, $data);
    ...
    if ($act == 'ajax_render_dashboard_table') {
        // Dynamic Filters
        $filterFields = $config->getFilterFields('Base_processos_schema');
        $filters = [];
        
        foreach ($filterFields as $f) {
            $key = $f['key'];
            $postKey = 'filter_' . str_replace([' ', '.'], '_', $key);
            $val = $_POST[$postKey] ?? '';
            if ($val !== '') {
                $filters[$key] = $val;
            }
        ...
        if ($fBusca) {
             // Global search: just check if value is in row
             $filters['callback'] = function($row) use ($fBusca, $checkDate) {
                  if (!$checkDate($row)) return false;
                  foreach ($row as $val) if (stripos((string)$val, $fBusca) !== false) return true;
                  return false;
             };
        } else {
 ...
 usort($processos, function($a, $b) use ($sortCol, $desc, $clientMap) {
        ...
        foreach($processos as $p) {
            ...
            $ports[] = get_value_ci($p, $sysPK);
        }
        $clientes = $db->findMany('Base_clientes.json', 'CPF', $cpfs);
        $clientMap = []; foreach ($clientes as $c) $clientMap[$c['CPF']] = $c['Nome'];
        $creditos = $db->findMany('Identificacao_cred.json', 'PORTABILIDADE', $ports); // Legacy lookup key in credit base? Or dynamic? Credit base is still using PORTABILIDADE usually.
        // Assuming Identificacao_cred uses standard key 'PORTABILIDADE' or whatever is in system config?
        // Ideally should use system config for Credit Base too if I want full consistency.
        // But for now, let's stick to PORTABILIDADE for Credit Base as I haven't refactored it completely.
        $creditoMap = []; foreach ($creditos as $c) {
             $k = get_value_ci($c, 'PORTABILIDADE') ?: get_value_ci($c, $sysPK);
             if($k) $creditoMap[$k] = $c;
        }

        $dashFields = $config->getDashboardFields('Base_processos_schema');

        // HEADER HTML
        $headerHtml = '<tr>';
        foreach ($dashFields as $f) {
            $colKey = $f['key'];
            $label = $f['label'];
            $icon = '<span class="sort-icon"><i class="fas fa-sort text-muted ms-1" style="opacity:0.3"></i></span>';
            if ($sortCol === $colKey) {
                $icon = '<span class="sort-icon">' . (($desc) ? '<i class="fas fa-sort-down text-dark ms-1"></i>' : '<i class="fas fa-sort-up text-dark ms-1"></i>') . '</span>';
            }
            $headerHtml .= '<th class="sortable-header" data-col="'.htmlspecialchars($colKey).'" onclick="setDashSort(\''.htmlspecialchars($colKey).'\')" style="cursor:pointer">' . htmlspecialchars($label) . ' ' . $icon . '</th>';
        }
        ...
        foreach($processos as $proc) {
            ...
            foreach ($dashFields as $f) {
                $val = get_value_ci($proc, $f['key']);
                
                // Special handling
                if (strcasecmp($f['key'], 'Nome') === 0 && empty($val)) {
                     $val = $nome;
                }
                
                if (strcasecmp($f['key'], $sysPK) === 0) {
                     echo '<td class="dashboard-text">' . htmlspecialchars($val);
                     if($cred) echo ' <i class="fas fa-sack-dollar money-bag ms-2" title="Crédito Identificado!"></i>';
                     echo '</td>';
                } elseif (strcasecmp($f['key'], 'STATUS') === 0) {
                     echo '<td><span class="badge bg-secondary status-badge">' . htmlspecialchars($val) . '</span></td>';
                } elseif (strcasecmp($f['key'], 'Nome_atendente') === 0) {
                     echo '<td class="dashboard-text">' . htmlspecialchars($val);
                     ...
                     echo '<td class="dashboard-text">' . htmlspecialchars($val) . '</td>';
                }
            }
        ...
        if(empty($processos)) echo '<tr><td colspan="'.(count($dashFields)+1).'" class="text-center text-muted py-4">Nenhum registro encontrado.</td></tr>';
        $html = ob_get_clean();
        ...
        ob_clean(); header('Content-Type: application/json');
        echo json_encode(['status' => 'ok', 'html' => $html, 'header_html' => $headerHtml, 'pagination' => $paginationHtml, 'page' => $res['page'], 'count' => $res['total']]);
        exit;
    }
    ...
    if ($act == 'ajax_get_process_full') {
        ...
        if ($file) {
            $res = $db->select($file, [$systemConfig->getPKField() => $port], 1, 1);
    ...
    if ($act == 'ajax_save_base_record') {
        ...
        if (stripos($base, 'client') !== false) $pk = 'CPF';
        if (stripos($base, 'agenc') !== false) $pk = 'AG';
        if ($base === 'Processos') $pk = $systemConfig->getPKField();
    ...
    if ($act == 'ajax_salvar_campo') {
        ...
        $fieldData = [
            ...
            'visible_in_dashboard' => isset($_POST['visible_in_dashboard']) ? true : false,
            'visible_in_filter' => isset($_POST['visible_in_filter']) ? true : false
        ];
    ...
    if ($act == 'ajax_render_config') {
        ...
        <div class="card card-custom p-4 mb-4">
            <h5 class="text-navy mb-3">Configurações do Sistema</h5>
            <form onsubmit="event.preventDefault(); submitConfigForm(this, 'ajax_save_system_config')">
                <input type="hidden" name="acao" value="save_system_config">
                <div class="row">
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Nome do Sistema</label>
                        <input type="text" name="app_name" class="form-control" value="<?= htmlspecialchars($systemConfig->getAppName()) ?>" required>
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Nome do ID Único (Chave Primária)</label>
                        <input type="text" name="pk_field" class="form-control" value="<?= htmlspecialchars($systemConfig->getPKField()) ?>" required>
                        <div class="form-text text-muted">Ex: Numero_Portabilidade, Protocolo, ID. Cuidado ao alterar se já houver dados.</div>
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Símbolo Monetário</label>
                        <input type="text" name="currency_symbol" class="form-control" value="<?= htmlspecialchars($systemConfig->getCurrencySymbol()) ?>" required>
                    </div>
                </div>
                <div class="d-flex justify-content-end">
                    <button class="btn btn-navy">Salvar Configurações</button>
                </div>
            </form>
        </div>
        ...
        <div class="row mb-5">
            <?php foreach(['Base_processos_schema' => 'Processos', 'Base_registros_schema' => 'Campos de Registros', 'Identificacao_cred.json' => 'Identificação'] as $file => $label): ?>
            <div class="col-md-3">
                <div class="card card-custom p-3 h-100">
                    ...
                    <ul class="list-group list-group-flush mb-3 sortable-list" data-file="<?= $file ?>">
                        <?php foreach($config->getFields($file) as $f): 
                            if(isset($f['deleted']) && $f['deleted']) continue;
                            // All fields are now editable/deletable per user request
...
// DOWNLOAD ANEXO
if (isset($_GET['acao']) && $_GET['acao'] == 'download_anexo') {
    $port = $_GET['port'] ?? '';
    $name = $_GET['name'] ?? '';
    
    if (!$port || !$name) die("Arquivo não especificado.");
    
    $path = $attachmentManager->getFilePath($port, $name);
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?= htmlspecialchars($systemConfig->getAppName()) ?></title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
...
<nav class="navbar navbar-expand-lg navbar-custom mb-4 sticky-top">
    <div class="container-fluid">
        <a class="navbar-brand" href="?p=dashboard" onclick="showPage('dashboard'); return false;"><i class="fas fa-chart-pie me-2"></i><?= htmlspecialchars($systemConfig->getAppName()) ?></a>
...
<div class="container-fluid px-4">
    ...
    <div id="page-dashboard" class="page-section" style="<?= $page=='dashboard'?'':'display:none' ?>">
        ...
        // Pre-fetch unique values for Select fields if needed
        $years = $_SESSION['selected_years'] ?? [date('Y')];
        $months = $_SESSION['selected_months'] ?? [(int)date('n')];
        ...
        foreach ($filterFields as $f) {
            if ($f['type'] === 'select' || $f['type'] === 'multiselect') {
                ...
                if (!empty($f['options'])) {
                    ...
                    // Fetch from DB if no options defined
                    foreach($dashFiles as $df) {
                         $vals = $db->getUniqueValues($df, $f['key']);
                         $opts = array_merge($opts, $vals);
                    }
                    $opts = array_unique($opts);
                    sort($opts);
                }
                $uniqueValues[$f['key']] = $opts;
            }
        }
        
        // Date filters are generic/global usually, or specific?
        // Retaining Search, Date Range, Month/Year as Global fixed filters for now, 
        ...
        <div class="card card-custom p-4 mb-4">
            ...
            <form onsubmit="filterDashboard(event)" class="row g-3" id="form_dashboard_filter">
                ...
                ?>
                <div class="col-md-2">
                    <label class="form-label small fw-bold"><?= htmlspecialchars($f['label']) ?></label>
                    <?php if (isset($uniqueValues[$key])): ?>
                        <select name="<?= $postKey ?>" class="form-select form-select-sm">
                            <option value="">Todos</option>
                            <?php foreach($uniqueValues[$key] as $opt): ?>
                                <option value="<?= htmlspecialchars($opt) ?>"><?= htmlspecialchars($opt) ?></option>
                            <?php endforeach; ?>
                        </select>
                    <?php elseif ($f['type'] === 'date'): ?>
                        <input type="date" name="<?= $postKey ?>" class="form-control form-control-sm">
                    <?php else: ?>
                        <input type="text" name="<?= $postKey ?>" class="form-control form-control-sm">
                    <?php endif; ?>
                </div>
                <?php endforeach; ?>
        ...
        <div class="row">
            ...
            <div class="col-md-12">
                <ul class="nav nav-tabs mb-3 justify-content-center">
                    ...
                    <li class="nav-item"><button type="button" class="nav-link" data-bs-toggle="tab" data-bs-target="#tab-anexos" onclick="listAnexos()">Anexos</button></li>
                </ul>
                ...
                <div class="tab-content">
                    <div class="tab-pane fade" id="tab-anexos">
                        <div class="card card-custom p-4">
                            <div class="d-flex justify-content-between mb-3">
                                <h5 class="text-navy">Arquivos Anexados</h5>
                                <div>
                                    <button class="btn btn-navy btn-sm" onclick="document.getElementById('anexo_file').click()"><i class="fas fa-plus me-1"></i> Adicionar Anexo</button>
                                    <input type="file" id="anexo_file" style="display:none" onchange="uploadAnexo()">
                                </div>
                            </div>
                            <div id="anexos_container">
                                <div class="text-center text-muted py-5"><i class="fas fa-info-circle me-1"></i> Selecione um anexo para visualizar.</div>
                            </div>
                        </div>
                    </div>
                        ...
                        <form method="POST" id="form_processo">
                            ...
                            foreach($procFields as $f): 
                                ...
                                $val = $getVal($processo, $f['key']);
                                    ...
                                    <div class="col-md-4 <?= $hideClass ?>" data-field-key="<?= $f['key'] ?>">
                                        ...
                                        ?>
                                        <?php if($f['key'] == $systemConfig->getPKField()): ?>
                                            <div class="input-group">
                                                <input type="text" name="<?= htmlspecialchars($f['key']) ?>" id="proc_port" class="form-control form-control-custom fw-bold" value="<?= htmlspecialchars($val ?: ($id ?? '')) ?>" required placeholder="Buscar/Criar...">
                            ...
                            <div class="d-flex justify-content-between">
                                <div id="div_delete_process">
                                    <?php if($processo): ?>
                                    <button type="button" class="btn btn-outline-danger" onclick="confirmDelete('<?= htmlspecialchars($processo[$systemConfig->getPKField()]) ?>')">Excluir Processo</button>
                                    <?php endif; ?>
                                </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-sm table-striped small">
                                    ...
                                    <tbody id="history_table_body">
                                        <?php 
                                        if ($processo) {
                                            $hist = $templates->getHistory($processo[$systemConfig->getPKField()]);
                                            foreach ($hist as $h) {
    ...
    <div id="page-base" class="page-section" style="<?= $page=='base'?'':'display:none' ?>">
       ...
       <div class="card card-custom p-3 mb-4">
           <ul class="nav nav-pills nav-fill gap-3" id="base-tab">
               <li class="nav-item">
                   <button class="nav-link active rounded-pill" id="tab-cred" onclick="switchBase('Identificacao_cred.json')">Identificação</button>
               </li>
               <li class="nav-item">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-check mb-3">
                            <input class="form-check-input" type="checkbox" name="visible_in_dashboard" id="field_visible_in_dashboard">
                            <label class="form-check-label" for="field_visible_in_dashboard">Visível no Dashboard</label>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-check mb-3">
                            <input class="form-check-input" type="checkbox" name="visible_in_filter" id="field_visible_in_filter">
                            <label class="form-check-label" for="field_visible_in_filter">Visível no Filtro</label>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    const CURRENCY_SYMBOL = "<?= htmlspecialchars($systemConfig->getCurrencySymbol()) ?>";
    ...
    function addFieldModal(file) { 
        ...
        document.getElementById('field_visible_in_dashboard').checked = false;
        document.getElementById('field_visible_in_filter').checked = false;
    ...
    function editField(f, file) { 
        ...
        document.getElementById('field_visible_in_dashboard').checked = (f.visible_in_dashboard === true || f.visible_in_dashboard === "true" || f.visible_in_dashboard === "1" || f.visible_in_dashboard === 1);
        document.getElementById('field_visible_in_filter').checked = (f.visible_in_filter === true || f.visible_in_filter === "true" || f.visible_in_filter === "1" || f.visible_in_filter === 1);
        new bootstrap.Modal(document.getElementById('modalField')).show(); 
    }
            ...
            if(res.found) { 
                ...
                if (res.credit_data) {
                    ...
                    var cardHtml = '<div class="card card-custom p-4 mb-4 border-warning border-3" id="dyn_credit_card">' + 
                        '<h5 class="text-warning fw-bold border-bottom pb-2"><i class="fas fa-coins me-2"></i>Identificação</h5>' +
                        '<div class="row g-3">';
                    for (var k in c) {
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        moneyInputs.forEach(input => {
            input.addEventListener('input', function(e) {
                ...
                e.target.value = CURRENCY_SYMBOL + ' ' + value;
            });
            // Initial format if value exists
            if(input.value && !input.value.includes(CURRENCY_SYMBOL)) {
    ...
    function filterDashboard(e, page) {
        ...
        .then(res => {
            ...
            if(res.status == 'ok') {
                ...
                if(res.header_html) document.getElementById('dash_table_head').innerHTML = res.header_html;
    ...
    function listAnexos() {
        ...
        var fd = new FormData();
        fd.append('acao', 'ajax_list_anexos');
        fd.append('port', port);
        ...
        .then(res => {
            if(res.status == 'ok') {
                ...
                if(res.files.length == 0) html = '<div class="alert alert-light text-center">Nenhum arquivo anexado.</div>';
                
                res.files.forEach(f => {
                    // Safe Port for URL
                    var safePort = port.replace(/[^a-zA-Z0-9_-]/g, '_');
                    
                    html += '<li class="list-group-item d-flex justify-content-between align-items-center">';
                    html += '<div>';
                    html += '<i class="fas fa-file me-2 text-muted"></i>';
                    html += '<strong>' + f.name + '</strong>';
                    html += '<br><small class="text-muted">' + f.size + ' bytes | ' + f.ext + ' | ' + f.date + '</small>';
                    html += '</div>';
                    html += '<div>';
                    html += '<button class="btn btn-sm btn-link text-info" onclick="renameAnexo(\''+f.name+'\')"><i class="fas fa-pen"></i></button>';
                    html += '<button class="btn btn-sm btn-link text-danger" onclick="deleteAnexo(\''+f.name+'\')"><i class="fas fa-trash"></i></button>';
                    html += '<a href="?acao=download_anexo&port=' + encodeURIComponent(port) + '&name=' + encodeURIComponent(f.name) + '" target="_blank" class="btn btn-sm btn-link text-primary"><i class="fas fa-download"></i></a>';
                    html += '</div>';
                    html += '</li>';
                });
                
                html += '</ul>';
                document.getElementById('anexos_container').innerHTML = html;
            }
        });
    }
    ...
    function uploadAnexo() {
        var port = document.getElementById('proc_port').value;
        var fileInput = document.getElementById('anexo_file');
        if(!port || fileInput.files.length === 0) return;
        
        var fd = new FormData();
        fd.append('acao', 'ajax_upload_anexo');
        fd.append('port', port);
        ...
        .then(res => {
            ...
            if(res.status == 'ok') {
                Swal.fire('Sucesso', 'Upload realizado!', 'success');
                listAnexos();
            } else {
                Swal.fire('Erro', res.message, 'error');
            }
    ...
    function deleteAnexo(name) {
        var port = document.getElementById('proc_port').value;
        Swal.fire({title: 'Excluir?', showCancelButton: true}).then(r => {
            if(r.isConfirmed) {
                var fd = new FormData();
                fd.append('acao', 'ajax_delete_anexo');
                fd.append('port', port);
                fd.append('name', name);
                fetch('', {method:'POST', body:fd}).then(r=>r.json()).then(res=>{
                    if(res.status=='ok') listAnexos();
    ...
    function renameAnexo(oldName) {
        var port = document.getElementById('proc_port').value;
        Swal.fire({input: 'text', inputValue: oldName, title: 'Renomear'}).then(r => {
            if(r.isConfirmed && r.value) {
                var fd = new FormData();
                fd.append('acao', 'ajax_rename_anexo');
                fd.append('port', port);
                fd.append('old', oldName);
                fd.append('new', r.value);
                fetch('', {method:'POST', body:fd}).then(r=>r.json()).then(res=>{
                    if(res.status=='ok') listAnexos();
                    else Swal.fire('Erro', res.message, 'error');
                });
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if(document.getElementById('dash_table_body')) {
            filterDashboard(null, 1);
        }
    });
</script>
