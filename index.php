            ...
            <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane fade" id="tab-email">
                        <div class="card card-custom p-4 mb-4">
                            <h5 class="text-navy mb-3"><i class="fas fa-envelope me-2"></i>E-mail</h5>
                            
                            <div class="row g-3 mb-3">
                                <div class="col-md-12">
                                    <label class="form-label">Tema</label>
                                    <div class="input-group">
                                        <select class="form-select" id="email_template_select" onchange="selectEmailTemplate(this.value)">
                                            <option value="">Selecione um tema...</option>
                                            <!-- Populated by JS -->
                                        </select>
                                        <button class="btn btn-outline-secondary" type="button" onclick="openTemplateModal()"><i class="fas fa-cog"></i></button>
                                    </div>
                                </div>
                            </div>

                            <div class="d-flex justify-content-end mb-3">
                                <button type="button" class="btn btn-sm btn-outline-primary" onclick="openEmailContactModal()"><i class="fas fa-user-plus me-1"></i> Cadastrar Contatos</button>
                            </div>

                            <div class="row g-3 mb-3">
                                <div class="col-md-6">
                                    <label class="form-label">Para</label>
                                    <input type="text" class="form-control" id="email_to">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">CC</label>
                                    <input type="text" class="form-control" id="email_cc">
                                </div>
                                <div class="col-md-12">
                            ...
                            <div class="table-responsive">
                                <table class="table table-sm table-striped small">
                                    <thead><tr>
                                        <th style="width: 30px;"><input type="checkbox" class="form-check-input" onclick="toggleAllChecks(this, 'email_hist_del')"></th>
                                        <th>Data/Hora</th>
                                        <th>Responsável</th>
                                        <th>Assunto</th>
                                        <th>Para</th>
...
<script>
    ...
    function saveEmailHistory(btn) {
        ...
        .then(res => {
            setButtonLoading(btn, false);
            if(res.status == 'ok') {
                Swal.fire('Sucesso', 'E-mail registrado no histórico.', 'success');
                loadProcess(port); 
            } else {
    ...
    function renderEmailHistory(history) {
        ...
        history.forEach(function(h) {
            if (h.TYPE === 'EMAIL') {
                ...
                tr.innerHTML = '<td><input type="checkbox" name="email_hist_del[]" class="form-check-input" value="'+idVal+'"></td>' + 
                               '<td>'+(h.DATA||'')+'</td><td>'+(h.USUARIO||'')+'</td><td>'+safeSubject+'</td><td>'+safeTo+'</td><td>'+safeCc+'</td><td>'+editBtn+'</td>';
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-hover">
                        <thead>
                            <tr>
                                <th>Contato</th>
                                <th class="text-center" style="width: 60px;">Para</th>
                                <th class="text-center" style="width: 60px;">CC</th>
                            </tr>
                        </thead>
...
function openEmailContactModal() {
    ...
    // Gather emails from form inputs
    var emails = new Set();
    
    // Scan process form for Client and Agency emails
    ...
    if (emails.size === 0) {
        tbody.innerHTML = '<tr><td colspan="3" class="text-center text-muted">Nenhum e-mail encontrado no cadastro.</td></tr>';
    } else {
        emails.forEach(email => {
            var tr = document.createElement('tr');
            tr.innerHTML = `
                <td>${email}</td>
                <td class="text-center"><input type="checkbox" class="form-check-input chk-email-to" value="${email}"></td>
                <td class="text-center"><input type="checkbox" class="form-check-input chk-email-cc" value="${email}"></td>
            `;
            tbody.appendChild(tr);
        });
    }
...
function applyEmailContacts() {
    ...
    document.querySelectorAll('.chk-email-to:checked').forEach(cb => to.push(cb.value));
    document.querySelectorAll('.chk-email-cc:checked').forEach(cb => cc.push(cb.value));
    
    var existingTo = document.getElementById('email_to').value.split(';').map(s=>s.trim()).filter(s=>s);
    var existingCc = document.getElementById('email_cc').value.split(';').map(s=>s.trim()).filter(s=>s);
    
    // Merge unique
    to.forEach(e => { if(!existingTo.includes(e)) existingTo.push(e); });
    cc.forEach(e => { if(!existingCc.includes(e)) existingCc.push(e); });
    
    document.getElementById('email_to').value = existingTo.join('; ');
    document.getElementById('email_cc').value = existingCc.join('; ');
...
function clearEmailSelection() {
    document.getElementById('email_template_select').value = '';
