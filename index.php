...
if (isset($_POST['acao']) && strpos($_POST['acao'], 'ajax_') === 0) {
    ...
    if ($act == 'ajax_get_process_filter_options') {
        // ... (Existing logic kept for legacy dashboard compatibility if needed, 
        // OR we can make this alias to new logic? 
        // Existing logic relies on Session Years/Months. 
        // Let's keep existing logic intact for Dashboard to avoid regression.)
        $years = $_SESSION['selected_years'] ?? [date('Y')];
        $months = $_SESSION['selected_months'] ?? [(int)date('n')];
        $files = $db->getProcessFiles($years, $months);
    ...
    if ($act == 'ajax_render_dashboard_table') {
        ...
        $checkDate = function($row) use ($fDataIni, $fDataFim, $fMes, $fAno, $visFilters, $fieldMap) {
            // Apply Dynamic Filters first (Type Aware)
            foreach($visFilters as $vk) {
                if ($vk === 'STATUS' || $vk === 'Nome_atendente' || $vk === 'DATA') continue;
                $valReq = $_POST['f_' . $vk] ?? '';
                if ($valReq !== '') {
                    $rowVal = get_value_ci($row, $vk);
                    $fType = $fieldMap[$vk] ?? 'text';
                    if (in_array($fType, ['text', 'textarea', 'custom'])) {
                        if (mb_stripos($rowVal, $valReq) === false) return false;
                    } else {
 ...
 usort($processos, function($a, $b) use ($sortCol, $desc, $clientMap) {
        ...
        foreach($procFields as $f) {
            ...
            if($show && ($f['type']??'') !== 'title' && !($f['deleted']??false)) {
                $dashColumns[] = ['key'=>$f['key'], 'label'=>$f['label']];
            }
        }
        // Order Logic
        ...
        foreach($processos as $proc) {
            $cpf = get_value_ci($proc, 'CPF'); $port = get_value_ci($proc, $ID_FIELD_KEY);
            ...
            if($cred) {
                 ...
                 else echo ' <span class="ms-2 text-success" title="Crédito Identificado!">' . $cIcon . '</span>';
            }
            echo '</td>';
            ...
            // Fixed Column: Atendente
            $ate = get_value_ci($proc, 'Nome_atendente');
            echo '<td class="dashboard-text">';
            echo htmlspecialchars($ate);
            $lastUpd = get_value_ci($proc, 'Ultima_Alteracao');
            if(!empty($lastUpd)) echo '<div class="small text-muted" style="font-size:0.75rem"><i class="fas fa-clock me-1"></i> ' . htmlspecialchars($lastUpd) . '</div>';
            ...
            foreach($dashColumns as $col) {
                $colKey = $col['key'];
                // Skip Fixed if present in Schema/List
                if(in_array($colKey, [$ID_FIELD_KEY, 'DATA', 'Nome_atendente'])) continue;
                
                $val = get_value_ci($proc, $colKey) ?: '';
                if ($colKey === 'Nome') $val = $nome; // Resolve Client Name
    ...
    if ($act == 'ajax_render_base_table') {
        ...
        $checkDynamicBase = function($row) use ($dynamicRules, $fieldMap) {
    ...
    if ($act == 'ajax_salvar_campo') {
        ...
        if ($oldKey) {
            ...
            // Check if we renamed the ID Field
            if ($oldKey === $ID_FIELD_KEY) {
                // Update Settings global configuration
                $settings->set('id_field_key', $key);
                $settings->set('id_label', $fieldData['label']);
                
                // DATA MIGRATION: Rename field in all process data files to maintain integrity
                // User requirement: "sem comprometer a integridade".
                // This ensures old records remain accessible with the new key.
                ...
                // SYNC IDENTIFICACAO: User requires ID to be unique and shared.
                // 1. Sync Settings
                $currIdentKey = $IDENT_ID_FIELD;
                $settings->set('identification_id_field', $key);
                
                // 2. Sync Schema (Identificacao_cred.json)
                // We update the field definition in Identificacao to match Processos ID
                $config->updateField('Identificacao_cred.json', $currIdentKey, $fieldData);
                
                // 3. Sync Data (Identificacao_cred.json)
                $identPath = $db->getPath('Identificacao_cred.json');
                if (file_exists($identPath)) {
                     ...
                     if (is_array($identJson)) {
                         ...
                         foreach($identJson as &$iRec) {
                             if (array_key_exists($currIdentKey, $iRec)) {
                                 // Rename key
                                 $iRec[$key] = $iRec[$currIdentKey];
                                 if ($key !== $currIdentKey) {
                                     unset($iRec[$currIdentKey]);
                                     $iMod = true;
                                 }
                             }
                         }
        ...
        if ($file === 'Base_processos_schema') {
             ...
             foreach($bFields as $bf) {
                 if (!empty($bf['show_column']) && !in_array($bf['key'], [$ID_FIELD_KEY, 'DATA', 'Nome_atendente', 'Nome', 'STATUS'])) {
             ...
             $filts = []; 
             foreach($bFields as $bf) {
                 if (!empty($bf['show_filter']) && !in_array($bf['key'], [$ID_FIELD_KEY, 'DATA', 'Nome_atendente', 'Nome', 'STATUS'])) {
                     $filts[] = $bf['key'];
                 }
             }
    ...
    if ($act == 'ajax_render_base_filters') {
         $baseFile = $_POST['file'] ?? 'Identificacao_cred.json'; 
         ...
         foreach($renderFilters as $bk) {
             ...
             $idAttr = '';
             // Map standard keys to IDs expected by JS population
...
<div class="container-fluid px-4">
    ...
    <div id="page-dashboard" class="page-section" style="<?= $page=='dashboard'?'':'display:none' ?>">
        ...
        $dynamicCheck = function($row) use ($visFilters, $fieldMap) {
            foreach($visFilters as $vk) {
                // Skip if fixed
                ...
                <table class="table table-hover table-custom align-middle" id="dash_table_head">
                    <thead><tr>
                        ...
                        foreach($procFields as $f) {
                            $show = (isset($f['show_dashboard_column']) && $f['show_dashboard_column']) || (!isset($f['show_dashboard_column']) && isset($f['show_column']) && $f['show_column']);
                            if($show && ($f['type']??'') !== 'title' && !($f['deleted']??false)) {
                                $dashCols[] = ['key'=>$f['key'], 'label'=>$f['label']];
                            }
                        }
                        // Order Logic
                        $dOrder = $settings->get('dashboard_columns_order', []);
    ...
    <div id="page-base" class="page-section" style="<?= $page=='base'?'':'display:none' ?>">
       ...
       <div class="card card-custom p-3 mb-4">
           <ul class="nav nav-pills nav-fill gap-3" id="base-tab">
               ...
               <li class="nav-item">
                   <button class="nav-link rounded-pill" id="tab-proc" onclick="switchBase('Processos')">Processos</button>
               </li>
           </ul>
       </div>
       ...
       <div class="card card-custom p-4 mb-4">
            ...
            <form onsubmit="filterBase(event)" class="row g-3 align-items-end" id="form_base_filter">
                ...
                // We use the default base (Identificacao) for the initial render. 
                $baseSchema = $config->getFields('Identificacao_cred.json');
                $renderFilters = [];
                foreach ($baseSchema as $f) {
                    $show = (isset($f['show_base_filter']) && $f['show_base_filter']) || (!isset($f['show_base_filter']) && isset($f['show_filter']) && $f['show_filter']);
                    if ($show) {
        ...
        <div class="row mb-5">
            ...
            <div class="col-md-3">
                <div class="card card-custom p-3 h-100">
                    <h5 class="text-navy">
                        <?php 
                            if ($file === 'Identificacao_cred.json') {
                                $iIcon = $IDENT_ICON ?: '<i class="fas fa-sack-dollar"></i>';
                                if(strpos($iIcon, '<') !== 0) echo '<span>'.$iIcon.'</span> ';
                                else echo $iIcon . ' ';
                            }
                        ?>
                        <?= $label ?> 
                        <?php if($file === 'Identificacao_cred.json'): ?>
                            <button class="btn btn-sm btn-outline-warning ms-2" onclick="changeIdentIcon()" title="Alterar Ícone"><i class="fas fa-icons"></i></button>
                        <?php endif; ?>
