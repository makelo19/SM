...
<script>
    ...
    var configAuthRequired = true;

    <?php
    $initAuth = true;
    if (file_exists('config_auth.json')) {
        $ac = json_decode(file_get_contents('config_auth.json'), true);
        if(isset($ac['required'])) $initAuth = $ac['required'];
    }
    echo "configAuthRequired = " . ($initAuth ? 'true' : 'false') . ";";
    ?>
    ...
    function requestConfigAccess() {
        ...
        if (sessionStorage.getItem('config_unlocked') === 'true') {
            window.isConfigUnlocked = true;
            showPage('config-hub');
            return;
        }

        if (window.isConfigUnlocked) {
            showPage('config-hub');
        } else {
