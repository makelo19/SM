    ...
    if ($act == 'ajax_salvar_campo') {
        ...
        if ($oldKey) {
            ...
            if ($oldKey === $ID_FIELD_KEY) {
                ...
                // 2. Sync Schema (Identificacao.json)
                // We update the field definition in Identificacao to match Processos ID
                $config->updateField('Identificacao.json', $currIdentKey, $fieldData);
                
                // 3. Sync Data (Identificacao.json)
        ...
        if ($file === 'Base_processos_schema') {
             ...
             if (!empty($filts)) {
                 ...
                 // Identificacao.json usually has NO schema file managed by Config class unless created manually.
                 // If managed, we sync.
             }
        }
        exit;
    ...
    <div id="page-base" class="page-section" style="<?= $page=='base'?'':'display:none' ?>">
       ...
       <div class="card card-custom p-3 mb-4">
           <ul class="nav nav-pills nav-fill gap-3" id="base-tab">
               <li class="nav-item">
                   <button class="nav-link active rounded-pill" id="tab-cred" onclick="switchBase('Identificacao.json')"><?= htmlspecialchars($IDENT_LABEL) ?></button>
               </li>
               <li class="nav-item">
       ...
       <div class="card card-custom p-4 mb-4">
            ...
            <form onsubmit="filterBase(event)" class="row g-3 align-items-end" id="form_base_filter">
                ...
                $baseSchema = $config->getFields('Identificacao.json');
                $renderFilters = [];
                foreach ($baseSchema as $f) {
                    $show = (isset($f['show_base_filter']) && $f['show_base_filter']) || (!isset($f['show_base_filter']) && isset($f['show_filter']) && $f['show_filter']);
                    if ($show) {
...
<script>
    ...
    // --- GENERIC BASE JS ---
    var currentBase = 'Identificacao.json';
